...
| Public booking cart order | `Booking/CartController::checkout` | public flow has no Branch selector | remains NULL | Phase 5 public Branch selection |
...
| Public hold/cart checkout | `HoldController`, `Booking/CartController` | none | remains NULL | Phase 5 selector |
...
4. Confirm both new columns are nullable and their foreign keys restrict Branch deletion.
...
Known unresolved rules: delivery ownership, public booking Branch choice, ambiguous historic POS/manual orders, expenses, and whether every future pickup fulfilment model implies business ownership. No inventory or multi-tenancy work is included, and no production backfill was executed during implementation.
