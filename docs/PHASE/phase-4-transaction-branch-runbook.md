...
The public Booking Shop derives its UX from the active booking-enabled Branch count. Zero Branches blocks booking; exactly one is auto-selected and uses the original four-step flow with a visible “Booking at” indicator; two or more require explicit selection and use the five-step flow. No first/default Branch is guessed in multi-Branch mode. Changing Branch navigates back to a clean Branch-only URL, which discards service, add-on, date/time, technician, and availability query state. Before final cart submission the public Branch list is refreshed, and an unavailable selection is invalidated rather than silently replaced. A booking cart can contain appointment lines for only one Branch, and checkout revalidates that Branch. This is attribution only: services, staff, schedules, and availability are deliberately still global until Phase 5 (the Branch ID is carried additively on availability requests but is not used to filter results).
