...
> **POS Booking transaction attribution correction (2026-08-30):** All is read-only for new operational Booking creation. Direct POS Appointments require an explicit authorized Branch; POS Book Service inherits its Branch-owned cart. `bookings.store_location_id` is persisted before side effects, Service/Staff/schedule eligibility is validated at that Branch, and deposit/settlement Orders inherit the Booking Branch. Settlement carts and Cash Shifts must match it. Sales Visual reads the persisted transaction Branch; legacy NULL remains Unassigned. Existing `branch-transactions:backfill` now prefers unique linked-Order evidence before any operator-approved default and leaves conflicting evidence unresolved. No Staff inference, cross-Branch reschedule, tenancy, or Phase 10 work is introduced.
...
> **Phase 9F POS operational catalogue correction (2026-08-28):** a specific Header Branch is now the single source of truth for all operational POS tabs. Products use authorized Product↔Branch availability and Branch inventory; Booking Products inherit availability through their existing linked Booking Service; Booking Services use `booking_service_store_location`; and Settlement uses persisted `bookings.store_location_id` (never Staff inference, NULL compatibility rows, or client filtering). Package definitions remain global because the architecture has no Package/Branch ownership or eligibility relationship. Lazy loading remains, while a Branch switch aborts and invalidates only Branch-owned tab state; see `phase-9f-pos-appointments-performance-hardening.md`.
>
> **Booking management/Header Branch completion (2026-08-28):** Booking Service and Booking Product identities remain global. Service applicability is the authorized `booking_service_store_location` pivot; Booking Product applicability is derived through its existing linked Booking Service. Service Category visibility is derived Category → Services → service Branch assignments, and Product Category visibility is derived Category → Products → linked Service → Branch. Specific Header Branch requests are authorized and filtered server-side. All Branches serializes each identity once with only accessible Branch metadata, using constrained eager loading rather than N+1 queries; specific tables hide redundant applicability context.
> The four CRM management tables follow the established Admin list contract: `branch_scope=all` for the accessible overview and `branch_store_location_id=<id>` for a specific Header Branch. The backend retains `store_location_id` as a compatibility alias for earlier POS/Booking consumers. All scope excludes identities applicable only to inaccessible Branches and displays the authorized `Available At` column; specific scope filters in SQL and hides that redundant column. Branch changes clear rows, reset page 1, and abort the previous request.
>
> **Pre-Multi-Branch migration-order compatibility (2026-08-30):** August 2026 analytics/query-index migrations execute before the January 2027 migrations that introduce `orders.store_location_id`, `store_location_product`, and Branch inventory on a restored production database. Early index migrations now skip any index whose complete table/column prerequisite is absent. `2027_01_06_000100_add_deferred_branch_query_indexes` runs after the Branch foundation and idempotently installs every skipped analytics/order index. This changes no data, attribution, or Branch semantics and is safe both for fresh installs and databases where the early indexes already exist.
>
> **POS cart lifecycle correction (2026-08-30):** the active operational cart identity is `(staff_user_id, store_location_id)`, so an operator may preserve one independent cart in every authorized POS Branch. Header Branch switching restores that Branch's cart and never moves, merges, or clears another Branch's cart. Every cart operation authorizes the explicit Branch; legacy NULL carts are excluded and remain for the reviewed `pos-branch:backfill` path. Checkout keeps the strict Cart = Order = open Cash Shift Branch invariant, and inventory mutation continues to use the persisted Order/Cart Branch.
> The backend continuation separates the heavyweight appointment detail calculation from the SQL-paginated lightweight calendar feed, adds request-scoped debug profiling and the justified `(store_location_id, start_at, id)` calendar-path index. Direct appointment actions and persisted Branch authorization are unchanged.
...
> **CRM catalogue list visibility (2026-08-25):** Product and Category list rows remain global identities. In **All Branches**, Products show their available `store_location_product` Branches and Categories show Branches derived through Category → Product → available `store_location_product`; both lists restrict metadata to backend-authorized accessible Branches. The Branch cell follows the established Admin list convention: full Branch names as comma-separated plain text, with `Unassigned` when empty. A specific Header Branch filters by the same `is_available` pivot rule and hides the redundant Branch column. Both APIs use eager/bulk queries rather than per-row Branch queries. CSV exports intentionally retain their existing global/specific catalogue semantics and do not mirror the new display-only Branch column.

> **CRM Promotion and Reward Product visibility (2026-08-25):** Promotion remains separate from Voucher. Its existing `is_online_enabled` flag represents the shared Online Ecommerce channel, while `promotion_store_location` is the authoritative offline POS applicability relationship. In **All Branches**, the Promotion list displays Online Ecommerce separately from the authorized offline Branch names (or `All POS Branches` when the relationship covers the complete accessible POS Branch set). A specific Header Branch scopes the list to Promotions usable online or at that Branch and renders compact `POS` / `Online Ecommerce` channel context without repeating the selected Branch name. The backend constrains its single eager-loaded Branch relationship to `StoreLocationAccessService` results, so inaccessible names are never serialized.
>
> Loyalty Reward Product definitions remain global rows in `loyalty_rewards`; no Branch owner or Reward/Branch pivot was added. Fulfilment is derived from the Reward's underlying global Product through available `store_location_product` rows. In **All Branches**, the CRM shows only accessible fulfilment Branches and uses `All Branches` only when all of the user's accessible Branches are available. A specific Header Branch filters Product rewards by that same availability pivot and hides the redundant applicability column. Both list contracts eager-load applicability in bounded queries rather than querying once per row.
>
> Voucher definition and eligibility remain global/shared. No Voucher schema or page was changed and no Branch applicability column was added. A Voucher's eventual usage transaction may retain its transaction Branch attribution, but the Voucher definition is not Branch-owned. This enhancement is part of the existing multi-Branch rollout and does **not** begin Phase 10.
...
**Implemented CRM Product/Category list contract:** Product identity remains global and Branch availability remains exclusively on `store_location_product`. Category identity also remains global: it has neither `store_location_id` nor a Category/Branch pivot, and its Branch visibility is derived using the existing available Product relationship. All-scope responses authorize the actor server-side (including the established `infra_core_x1` bypass), constrain Product Branch eager loading to accessible available relationships, and build Category Branch metadata with one grouped join for the current page. Specific-scope lists use the same `store_location_product.is_available = true` predicate and omit Branch metadata in the UI. Header changes retain abort/generation guards so late responses cannot replace the latest scope, while conditional column counts keep loading and empty rows aligned.
...
### Wishlist report inventory and ranking semantics

- Wishlist identity remains global and product-level: `customer_wishlist_items` stores the customer and product, while `guest_wishlist_items` stores a session token and product. Neither table stores a variant, SKU snapshot, Store Location or Branch. Customer/session uniqueness is enforced per product.
- Current Ecommerce availability continues to use the global catalogue stock source (`products.stock` for a simple product and active `product_variants.stock` for a variant product), including untracked-stock and derived bundle availability rules. The Wishlist report does not consult `store_location_product_inventories` and does not attribute intent to a Branch.
- A simple tracked product is in stock above zero and out of stock at zero or below. For a product with active purchasable variants, all available means **In stock**, a mixture means **Some variants out of stock (unavailable/total)**, and none available means **Out of stock**. A partial shortage is not counted by **Out-of-stock Products With Wishlist Demand**; that card remains a product-level count, so its label is unchanged.
- The initial product-grain report response carries only `stock_status`, `current_stock`, `has_variants`, `variant_count`, and `out_of_stock_variant_count`. It obtains variant counts in one grouped join, rather than querying each row. An eye action lazily calls the inventory-detail endpoint for one selected variant product and displays active variant name, SKU, current stock and availability. Because demand has no variant identity, the modal explicitly says that wishlist demand applies to the product and does not invent variant wishlist counts.
- The top-product calculation takes the maximum after all report filters (including Product Search and date bounds), then counts every product at that maximum. One result displays its product name; multiple results display a tie count and per-product wish count; no results display **No wishlist data**. A separate tie modal was intentionally omitted because the already-filtered table contains the tied rows and the variant inventory modal is the useful drill-down.

## Phase 9C — production-readiness hardening

Phase 9C closes confirmed reporting P0/P1 gaps without changing the Branch architecture: invoice PDFs authorize persisted Order Branches; Global Product Master CSV excludes physical stock; operational Order defaults are fail-safe; Sales/Booking expose explicit legacy Unassigned totals; low stock is evaluated per available accessible Branch with missing inventory treated as zero; mixed inventory cutover notifications fail safely; and Branch-changing report requests cancel stale work. Transaction-unlinked commission snapshots, global company Expense allocation, and non-existent Points/Voucher/Promotion usage workspaces remain explicitly deferred rather than guessed. Detailed implementation, test, and manual-QA status is maintained in `docs/PHASE/phase-9-reporting-runbook.md`.

> **POS Appointments All Branches completion (2026-08-30):** All Branches is now a cross-Branch appointment overview over the authenticated `StoreLocationAccessService` set, served by the Phase 9F lightweight calendar query. Rows carry persisted Branch metadata and calendar blocks show it in All scope without duplicating Staff columns. Detail, Edit Settlement, reschedule, lifecycle actions and payment links use and authorize the persisted Booking Branch without switching the Header; reschedule does not move Branches. ALL defines no new action policy: Mark Completed, cancel/no-show/late-cancel, service-photo management, checkout/finalization and package consumption retain the pre-ALL open-shift gate, evaluated against the persisted appointment Branch; Edit Settlement, reschedule, confirmation email and payment links retain their pre-ALL no-shift behavior. All itself never owns a Cash Shift. Checkout/finalization and package consumption require the exact appointment Branch's open shift, reject another Branch's shift, and reject unresolved legacy NULL safely. The documented All-only Unassigned visibility compatibility remains; specific Branch scope excludes NULL.

## Phase 9G — Branch-aware POS payment methods (2026-08-31)

POS availability is explicitly separate from global Online Payment Gateway, Bank Account and Billplz configuration. Global POS identities relate to existing `store_locations` through Branch availability/order rows; split and Auto Calculate behavior is transaction-local UI state. Header ALL is never a transaction/configuration Branch: direct POS uses the persisted Cart Branch and Appointment Settlement uses the persisted Booking Branch. Backend validation rejects disabled methods while original split reconciliation operates independently of unchanged exact-Branch Cash Shift and member-only Customer Balance rules. Existing Orders are not rewritten. New/unconfigured Branches safely expose Cash only; legacy PNG is explicitly materialized by the dry-run/force initializer documented in [Phase 9G](phase-9g-pos-payment-methods-enhancement.md).

### Phase 9G fresh bootstrap separation (2026-08-31)

Fresh install and production reconciliation are now distinct. `php artisan migrate:fresh --seed` uses the configured default Branch code, creates an explicit four-method POS availability/order configuration, and creates no second QA Branch, demo catalogue, Expense, Booking, Order, or multi-Branch fixture. Normal re-seeding preserves customized POS rows. Existing production still uses migrate + the permission seeder + reviewed `pos-payment-methods:initialize --store-code=PNG` dry-run/force; migrations and DatabaseSeeder do not run that reconciliation. Later Branches remain unconfigured Cash-only until explicitly configured. The complete automatic-seeder classification and exact commands are maintained in [Phase 9G](phase-9g-pos-payment-methods-enhancement.md#phase-9g-bootstrap-paths-and-databaseseeder-audit-2026-08-31).
...
## Multi-Branch email and notification settings (2026-09-12)

SMTP/Mailgun infrastructure and sender identity remain platform-global. Operational configuration is persisted one-to-one in `branch_notification_settings` for each concrete `StoreLocation`: Booking reminder/feedback schedules, Booking payment-proof recipients, Branch daily pending-work summaries, and Branch low-stock recipients. Existing Branches are initialized explicitly with the idempotent `branch-email-settings:initialize` command; future Branches receive persisted initial rows on creation. Runtime never substitutes PNG, first Branch, Header scope, authenticated user, Staff assignment, or the initial recipient.

Booking customer messages take queue-safe venue presentation from the persisted `bookings.store_location_id` relationship. Legacy NULL bookings display unavailable venue and are excluded from Branch-routed internal proof alerts. POS receipt identity continues to come from `InvoiceService` using persisted Order Branch. Customer-facing Ecommerce shipment/payment identity, verification, and password reset remain global. Mixed Ecommerce pending Orders participate in each persisted fulfilment Branch summary using only that Branch's item subtotal, so NULL Order ownership is not treated as Unassigned and whole revenue is not duplicated.
