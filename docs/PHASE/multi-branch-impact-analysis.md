...
> **CRM catalogue list visibility (2026-08-25):** Product and Category list rows remain global identities. In **All Branches**, Products show their available `store_location_product` Branches and Categories show Branches derived through Category → Product → available `store_location_product`; both lists restrict metadata to backend-authorized accessible Branches. A specific Header Branch filters by the same `is_available` pivot rule and hides the redundant Branches column. Both APIs use eager/bulk queries rather than per-row Branch queries. CSV exports intentionally retain their existing global/specific catalogue semantics and do not mirror the new display-only Branches column.
