...
Branch-scoped Product listing is intentionally driven by `store_location_product.is_available`, not inventory quantity. The PNG inventory migration therefore does not make Products visible by itself; the separate idempotent `product-branch:backfill --store-code=PNG` command adds only missing legacy availability assignments while preserving explicit and other-Branch assignments. Category masters remain global and visible in every Header context; only Product-derived counts are scoped by the same availability rule, including zero-count Categories.
