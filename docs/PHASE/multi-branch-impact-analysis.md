...
## Phase 9F POS checkout performance and Branch impact

The checkout catalogue is now a dedicated operational Branch read, not the global ecommerce management Product DTO. Every request requires `store_location_id`, reuses `StoreLocationAccessService` authorization, filters `store_location_product.is_available`, and obtains quantities from authoritative `store_location_product_inventories`; movement history is not aggregated. Header Branch switching aborts/invalidates the old catalogue and clears its cards before requesting the next Branch, while global Member and Package ownership semantics remain unchanged.

Cash Shift and inventory stay fresh per Branch and are not cached. Service, booking Product, Package and settlement lists no longer contend with initial Product usability and load on their tabs. Member detail/history/wallet/eligibility remain interaction-driven and printer state optional. Existing inventory indexes match the snapshot lookup, so no migration, business data backfill, Multi-Tenancy concept, or Appointments rework was introduced. Staging timings, query diagnostics and PostgreSQL plans remain the Phase 9F release gate.
