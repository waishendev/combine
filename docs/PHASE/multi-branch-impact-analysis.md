...
> **POS Appointments All Branches completion (2026-08-30):** All Branches is now a cross-Branch appointment overview over the authenticated `StoreLocationAccessService` set, served by the Phase 9F lightweight calendar query. Rows carry persisted Branch metadata and calendar blocks show it in All scope without duplicating Staff columns. Detail, Edit Settlement, reschedule, cancellation/no-show and payment links use and authorize the persisted Booking Branch without switching the Header; reschedule does not move Branches. All itself never owns a Cash Shift. Checkout/finalization and package consumption require the exact appointment Branch's open shift, reject another Branch's shift, and reject unresolved legacy NULL safely. The documented All-only Unassigned visibility compatibility remains; specific Branch scope excludes NULL.
