...
Configure the shared account with `MULTI_BRANCH_SEED_SHARED_ADMIN_EMAIL` and `MULTI_BRANCH_SEED_SHARED_ADMIN_USERNAME`; it uses `MULTI_BRANCH_SEED_ADMIN_PASSWORD`, like the two Branch-specific Admins. `MULTI_BRANCH_SEED_QA_DATA=false` retains Branches/Admins but omits the QA data pass. Individual setup can also be run explicitly with `php artisan db:seed --class=FreshInstallBranchOneSeeder`, `FreshInstallBranchTwoSeeder`, or `FreshInstallSharedBranchAdminSeeder` after roles/migrations exist. An unknown profile fails clearly. Fresh Branch/Admin and QA fixture seeders refuse production execution. Change the configured emails, usernames and password outside local/QA environments. Default Expense Categories and demo Expenses are now attributed to Branch One rather than creating legacy `NULL` ownership.
