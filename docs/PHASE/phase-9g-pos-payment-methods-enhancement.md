...
After UX review, Phase 9G Branch configuration was intentionally reduced to enabled methods and sort order. The obsolete one-to-one Branch split-settings table is removed by corrective migration `2027_03_04_000002`, which is safe for databases that already ran the original Phase 9G migration. POS Checkout retains its original per-transaction Auto Calculate Split checkbox, defaulting checked, and Appointment Settlement now exposes the same local control over its existing multi-method amount entry. Both surfaces filter and order methods by the persisted Cart/Booking Branch. The backend no longer applies a Branch split policy, while all original reconciliation, Cash Shift, Customer Balance, Branch ownership, and enabled-method enforcement remains.

## Appointment Settlement Auto Calculate Split

POS Checkout and Appointment Settlement Checkout Confirmation now share the same transaction-local remainder-distribution helper and checked-by-default Auto Calculate Split control. Editing one method while one other method has a value assigns that other method the exact remaining cents; multiple populated methods share the remainder proportionally with the final row absorbing rounding. Turning the checkbox off preserves manual values and the existing total-paid validation remains authoritative. The control resets to checked for each newly opened settlement and is never loaded from or saved to Branch configuration. The persisted appointment Branch continues to determine only enabled methods and display order; Customer Balance caps/member validation and exact-Branch Cash Shift enforcement are unchanged.
