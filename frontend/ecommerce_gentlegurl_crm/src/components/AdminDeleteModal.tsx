    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md mx-auto bg-white rounded-lg shadow-lg">
        <div className="flex items-center justify-between border-b border-gray-300 px-5 py-4">
          <h2 className="text-lg font-semibold">{t('admin.deleteTitle')}</h2>
          <button
            onClick={() => {
              if (!submitting) onClose()
            }}
        ...
        <div className="px-5 py-4 space-y-4">
          <p className="text-sm text-gray-700">{t('admin.deletePrompt')}</p>
          <div className="rounded-md bg-yellow-100 px-4 py-3">
            <p className="text-sm font-semibold text-yellow-800">{admin.username}</p>
            <p className="text-xs text-yellow-800">{admin.email}</p>
          </div>

          {isOwnAccount && (
            <div className="rounded-md border border-yellow-200 bg-yellow-50 px-4 py-3 text-sm text-yellow-800">
              <p className="font-semibold">{t('admin.deleteSelfWarningTitle')}</p>
              <p>{t('admin.deleteSelfWarningMessage')}</p>
            </div>
          )}
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            >
              {submitting ? t('admin.deleting') : t('admin.deleteAction')}
            </button>
          </div>
        </div>
      </div>
