...
export default function RewardVoucherEditModal({
  ...
  useEffect(() => {
    ...
    const fetchVoucher = async () => {
      try {
        const res = await fetch(`/api/proxy/ecommerce/vouchers/${reward.voucherId}`, {
          cache: 'no-store',
          signal: controller.signal,
          headers: {
        ...
        const payload =
          data && typeof data === 'object' && 'data' in data
            ? ((data as { data?: { scope_type?: string | null; products?: Array<{ id: number }>; categories?: Array<{ id: number }> } }).data ?? null)
            : null
        ...
        setForm((prev) => ({
          ...prev,
          scopeType:
            payload.scope_type === 'products' || payload.scope_type === 'categories'
              ? payload.scope_type
              : 'all',
          productIds: Array.isArray(payload.products) ? payload.products.map((item) => item.id) : [],
          categoryIds: Array.isArray(payload.categories) ? payload.categories.map((item) => item.id) : [],
        }))
      } catch (err) {
        if (!(err instanceof DOMException && err.name === 'AbortError')) {
          console.error(err)
  ...
  return (
    ...
    >
        <form id="reward-voucher-edit-form" onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
