...
type Props = {
  show: boolean
  selectedCategories: CategoryRowData[]
  onClose: () => void
  onSuccess: () => Promise<void> | void
}

export default function CategoryBulkUpdateModal({
  show,
  selectedCategories,
  onClose,
  onSuccess,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/proxy/ecommerce/categories/bulk', {
        method: 'PUT',
          ...
          {selectedCategories.map((c) => (
            <div key={c.id} className="mb-3 border-b border-gray-200 pb-2 last:mb-0 last:border-none last:pb-0">
              ...
              <div className="text-xs text-gray-500">
                #{c.id} • {c.slug} • {c.isActive ? 'Active' : 'Inactive'} • POS filter: {c.showInPosFilter ? 'Yes' : 'No'}
              </div>
            </div>
          ))}
        </div>
