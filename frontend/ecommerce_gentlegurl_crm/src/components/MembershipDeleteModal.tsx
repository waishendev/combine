...
export default function MembershipDeleteModal({
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/proxy/ecommerce/membership-tiers/${membership.id}`, {
        method: 'DELETE',
        headers: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md mx-auto bg-white rounded-lg shadow-lg">
        <div className="flex items-center justify-between border-b border-gray-300 px-5 py-4">
          <h2 className="text-lg font-semibold">Delete Membership Tier</h2>
          <button
            onClick={() => {
              if (!submitting) onClose()
            }}
        ...
        <div className="px-5 py-4 space-y-4">
          <p className="text-sm text-gray-700">Are you sure you want to delete this membership tier?</p>
          <div className="rounded-md bg-yellow-100 px-4 py-3">
            <p className="text-sm font-semibold text-yellow-800">{membership.displayName}</p>
            <p className="text-xs text-yellow-800">Tier: {membership.tier}</p>
          </div>
