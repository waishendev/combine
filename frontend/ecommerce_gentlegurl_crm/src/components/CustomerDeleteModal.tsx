  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(t('customer.deleteError'))
        return
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md mx-auto bg-white rounded-lg shadow-lg">
        <div className="flex items-center justify-between border-b border-gray-300 px-5 py-4">
          <h2 className="text-lg font-semibold">{t('customer.deleteTitle')}</h2>
          <button
            onClick={() => {
              if (!submitting) onClose()
            }}
        ...
        <div className="px-5 py-4 space-y-4">
          <p className="text-sm text-gray-700">{t('customer.deletePrompt')}</p>
          <div className="rounded-md bg-yellow-100 px-4 py-3">
            <p className="text-sm font-semibold text-yellow-800">{customer.name}</p>
            <p className="text-xs text-yellow-800">{customer.email}</p>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button
              ...
              disabled={submitting}
            >
              {submitting ? t('customer.deleting') : t('customer.deleteAction')}
            </button>
          </div>
        </div>
      </div>
