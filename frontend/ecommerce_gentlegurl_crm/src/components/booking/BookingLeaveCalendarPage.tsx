...
export default function BookingLeaveCalendarPage({ permissions = [] }: BookingLeaveCalendarPageProps) {
  ...
  const closeCancelOffDay = () => {
    if (actionLoadingId != null) return
    setCancelTarget(null)
    setError(null)
  }

  const confirmCancelOffDay = async () => {
    if (!cancelTarget) return

    setError(null)
    setActionLoadingId(cancelTarget.id)
    try {
      const res = await fetch(`/api/proxy/admin/booking/off-days/${cancelTarget.id}/cancel`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      })
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-1.5 shadow-sm sm:p-4">
        {calendarLoading ? (
          <div
            role="status"
            aria-live="polite"
            className="flex min-h-[22rem] flex-col items-center justify-center gap-3 rounded-lg bg-slate-50 px-4 py-12 sm:min-h-[28rem]"
          >
            ...
            <p className="text-sm font-medium text-slate-500">Loading leave calendar…</p>
          </div>
        ) : (
          <>
      ...
      {cancelTarget && (
        <CrmFormModalShell
          title="Cancel Off Day"
          onClose={closeCancelOffDay}
          closeDisabled={actionLoadingId === cancelTarget.id}
          footer={
            <>
              <button
                type="button"
                className="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-200 disabled:opacity-50"
                onClick={closeCancelOffDay}
              ...
              <button
                ...
                onClick={() => void confirmCancelOffDay()}
                disabled={actionLoadingId === cancelTarget.id}
              >
                {actionLoadingId === cancelTarget.id ? 'Cancelling…' : 'Confirm Cancel'}
              </button>
            </>
          }
        >
          <div className="space-y-3 px-4 py-4 text-sm text-slate-700">
            <p>
              ...
              <span className="font-semibold text-slate-900">
                {cancelTarget.staff?.name ?? `Staff #${cancelTarget.staff_id}`}
              </span>
              ?
            </p>
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
              ...
              {isAllBranches ? (
                <div className="mt-1">
                  ...
                  {cancelTarget.store_location?.name ?? (cancelTarget.store_location_id ? 'Unknown Branch' : 'Unassigned')}
                </div>
              ) : null}
            </div>
            <p className="text-xs text-slate-500">This removes the approved off day from the leave calendar.</p>
            {error ? <LeaveCalendarErrorBanner message={error} /> : null}
          </div>
        </CrmFormModalShell>
      )}
