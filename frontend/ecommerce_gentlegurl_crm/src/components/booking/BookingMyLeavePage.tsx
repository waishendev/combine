    ...
    <div className="space-y-6">
      <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <form className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-3" onSubmit={applyLeave}>
          ...
          <input className="rounded-md border border-slate-300 px-3 py-2 text-sm" type="date" value={form.start_date} onChange={(e) => setForm((prev) => ({ ...prev, start_date: e.target.value }))} required />
          <input className="rounded-md border border-slate-300 px-3 py-2 text-sm" type="date" value={form.end_date} onChange={(e) => setForm((prev) => ({ ...prev, end_date: e.target.value }))} required />
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {!loading && requests.length === 0 && (
                <tr><td colSpan={7} className="px-2 py-6 text-center text-slate-500">No leave requests yet.</td></tr>
              )}
              {requests.map((row) => (
