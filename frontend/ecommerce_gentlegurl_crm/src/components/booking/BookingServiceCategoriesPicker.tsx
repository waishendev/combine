...
export default function BookingServiceCategoriesPicker({
  categories,
  value,
  onChange,
  disabled = false,
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter(
      (c) =>
        ...
        (c.cn_name ?? '').toLowerCase().includes(q),
    )
  }, [sorted, searchQuery])
  ...
  const formatLabel = (category: BookingServiceCategoryOption) =>
    ...
    if (value.length <= 2) {
      return (
        <span className="flex items-center gap-2 truncate text-gray-700">
          ...
          {selectedCategories.map((c) => formatLabel(c)).join(', ')}
        </span>
      )
    }
    return (
      <span className="flex items-center gap-2 text-gray-700">
        <i className="fa-solid fa-check-circle text-xs text-blue-600" />
        <span className="font-medium">{value.length} categories selected</span>
      </span>
    )
  }

  const allFilteredSelected =
    filtered.length > 0 && filtered.every((c) => value.includes(c.id))
  ...
  return (
    <div ref={rootRef} className="space-y-2">
      <label className="block text-sm font-medium text-gray-700">
        ...
        {value.length > 0 && (
          <span className="ml-2 text-xs font-normal text-gray-500">({value.length} selected)</span>
        )}
      </label>
          ...
          <div className="absolute z-30 mt-2 w-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl">
            ...
            <div className="max-h-64 overflow-y-auto">
              {loading ? (
                  ...
                  {filtered.map((category) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isSelected && (
                          <i className="fa-solid fa-check-circle flex-shrink-0 text-xs text-blue-600" />
                        )}
                      </label>
                    )
                  })}
                ...
                <div className="p-6 text-center">
                  <i className="fa-solid fa-folder-open mb-2 text-2xl text-gray-300" />
                  <p className="text-sm text-gray-500">
                    {searchQuery ? 'No categories match your search.' : 'No active categories found.'}
                  </p>
                </div>
              )}
            </div>
