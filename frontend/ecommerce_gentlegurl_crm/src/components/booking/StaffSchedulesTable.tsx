...
export default function StaffSchedulesTable({
  ...
  const handleExportCsv = async () => {
    setIsExporting(true)
    try {
      const res = await fetch('/api/proxy/admin/booking/staff-schedules/export', { cache: 'no-store' })
      if (!res.ok) {
      ...
      const fileName = fileNameMatch?.[1] ?? `booking-staff-schedules-export_${new Date().toISOString().replace(/[:.]/g, '-')}.csv`
  ...
  const handleImportCsvFile = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch('/api/proxy/admin/booking/staff-schedules/import', {
        method: 'POST',
        body: formData,
      })
