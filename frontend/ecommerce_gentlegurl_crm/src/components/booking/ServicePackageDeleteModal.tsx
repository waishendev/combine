...
interface ServicePackageDeleteModalProps {
  servicePackage: ServicePackage
  onClose: () => void
  onDeleted: () => Promise<void> | void
}

export default function ServicePackageDeleteModal({
  servicePackage,
  onClose,
  onDeleted,
}: ServicePackageDeleteModalProps) {
  const { t } = useI18n()
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/proxy/service-packages/${servicePackage.id}`, {
        method: 'DELETE',
        headers: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative mx-auto w-full max-w-md rounded-lg bg-white shadow-lg">
        <div className="flex items-center justify-between border-b border-gray-300 px-5 py-4">
          <h2 className="text-lg font-semibold">Delete Service Package</h2>
          <button
            onClick={() => {
              if (!submitting) onClose()
            }}
        ...
        <div className="space-y-4 px-5 py-4 text-sm text-gray-700">
          <p>
            Are you sure you want to delete <strong>{servicePackage.name}</strong>?
          </p>
