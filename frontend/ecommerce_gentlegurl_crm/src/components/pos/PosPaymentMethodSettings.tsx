...
export default function PosPaymentMethodSettings() {
  ...
  return <div className="space-y-5">
    ...
    <div className="rounded-xl border bg-white p-4"><label className="mr-6"><input type="checkbox" className="mr-2" checked={config.allow_split_payment} onChange={e => setConfig({...config, allow_split_payment: e.target.checked})}/>Allow Split Payment</label><label><input type="checkbox" className="mr-2" checked={config.auto_calculate_split} onChange={e => setConfig({...config, auto_calculate_split: e.target.checked})}/>Auto Calculate Split by default</label></div>
