...
interface RewardProductDeleteModalProps {
  title: string
  rewardId: number
  productId: number
  onClose: () => void
...
export default function RewardProductDeleteModal({
  title,
  rewardId,
  productId,
  onClose,
  onDeleted,
}: RewardProductDeleteModalProps) {
  const { t } = useI18n()
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const productRes = await fetch(`/api/proxy/ecommerce/products/${productId}`, {
        method: 'DELETE',
      })
