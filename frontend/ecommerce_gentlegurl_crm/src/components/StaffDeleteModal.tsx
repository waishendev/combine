...
export default function StaffDeleteModal({
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/proxy/staffs/${staff.id}`, {
        method: 'DELETE',
        headers: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md mx-auto bg-white rounded-lg shadow-lg">
        ...
        <div className="px-5 py-4 space-y-4">
          <p className="text-sm text-gray-700">Are you sure you want to deactivate this staff member?</p>
          <div className="rounded-md bg-yellow-100 px-4 py-3">
            <p className="text-sm font-semibold text-yellow-800">{staff.name}</p>
            <p className="text-xs text-yellow-800">{staff.email}</p>
          </div>
