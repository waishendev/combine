    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          ...
          <tbody>
            {loading ? <TableLoadingRow colSpan={mode === 'booking' ? 8 : 7} /> : rows.length === 0 ? <TableEmptyState colSpan={mode === 'booking' ? 8 : 7} /> : mode === 'ecommerce' ? (rows as EcommerceRow[]).map((row) => <tr key={row.customer_id}><td className="px-4 py-2 border border-gray-200 font-medium">{row.customer_name}</td><td className="px-4 py-2 border border-gray-200">{row.customer_email ?? '—'}</td><td className="px-4 py-2 border border-gray-200">{row.orders_count}</td><td className="px-4 py-2 border border-gray-200">{row.items_count}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.revenue)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.cogs)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.gross_profit)}</td></tr>) : (rows as BookingRow[]).map((row) => <tr key={row.customer_id}><td className="px-4 py-2 border border-gray-200 font-medium">{row.customer_name}</td><td className="px-4 py-2 border border-gray-200">{row.customer_email ?? '—'}</td><td className="px-4 py-2 border border-gray-200">{row.transactions_count}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.booking_deposit_amount)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.booking_settlement_amount)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.addon_revenue ?? 0)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.package_purchase_amount)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.total_revenue)}</td></tr>)}
          </tbody>
          <tfoot>
