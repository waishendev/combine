    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-slate-300/70">
            <tr>
              {mode === 'ecommerce'
                ...
                : ['Customer', 'Email', 'Transactions', 'Booking Deposit', 'Booking Settlement', 'Add-on Revenue', 'Package Purchase', 'Total Revenue'].map((h) => (
                    <th key={h} className="px-4 py-2 font-semibold text-left text-gray-600">
                      {reportTableColumnHeader(h)}
                    </th>
                  ))}
          ...
          <tbody>
            {loading ? <TableLoadingRow colSpan={mode === 'ecommerce' ? 7 : 8} /> : rows.length === 0 ? <TableEmptyState colSpan={mode === 'ecommerce' ? 7 : 8} /> : mode === 'ecommerce' ? (rows as EcommerceRow[]).map((row) => <tr key={row.customer_id}><td className="px-4 py-2 border border-gray-200 font-medium">{row.customer_name}</td><td className="px-4 py-2 border border-gray-200">{row.customer_email ?? '—'}</td><td className="px-4 py-2 border border-gray-200">{row.orders_count}</td><td className="px-4 py-2 border border-gray-200">{row.items_count}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.revenue)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.cogs)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.gross_profit)}</td></tr>) : (rows as BookingRow[]).map((row) => <tr key={row.customer_id}><td className="px-4 py-2 border border-gray-200 font-medium">{row.customer_name}</td><td className="px-4 py-2 border border-gray-200">{row.customer_email ?? '—'}</td><td className="px-4 py-2 border border-gray-200">{row.transactions_count}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.booking_deposit_amount)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.booking_settlement_amount)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.addon_revenue)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.package_purchase_amount)}</td><td className="px-4 py-2 border border-gray-200">RM {formatAmount(row.total_revenue)}</td></tr>)}
          </tbody>
          <tfoot>
            <tr className="bg-gray-100 font-semibold"><td className="border border-gray-300 px-4 py-2 text-left">Page Totals</td><td className="border border-gray-300 px-4 py-2">—</td><td className="border border-gray-300 px-4 py-2">—</td>{mode === 'ecommerce' ? <><td className="border border-gray-300 px-4 py-2">—</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(totalsPage.revenue ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(totalsPage.cogs ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(totalsPage.gross_profit ?? 0)}</td></> : <><td className="border border-gray-300 px-4 py-2">RM {formatAmount(totalsPage.booking_deposit_amount ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(totalsPage.booking_settlement_amount ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(totalsPage.addon_revenue ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(totalsPage.package_purchase_amount ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(totalsPage.total_revenue ?? 0)}</td></>}</tr>
            <tr className="bg-gray-100 font-bold"><td className="border border-gray-300 px-4 py-2 text-left">Grand Totals</td><td className="border border-gray-300 px-4 py-2">—</td><td className="border border-gray-300 px-4 py-2">—</td>{mode === 'ecommerce' ? <><td className="border border-gray-300 px-4 py-2">—</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(grandTotals.revenue ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(grandTotals.cogs ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(grandTotals.gross_profit ?? 0)}</td></> : <><td className="border border-gray-300 px-4 py-2">RM {formatAmount(grandTotals.booking_deposit_amount ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(grandTotals.booking_settlement_amount ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(grandTotals.addon_revenue ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(grandTotals.package_purchase_amount ?? 0)}</td><td className="border border-gray-300 px-4 py-2">RM {formatAmount(grandTotals.total_revenue ?? 0)}</td></>}</tr>
          </tfoot>
        </table>
      </div>
