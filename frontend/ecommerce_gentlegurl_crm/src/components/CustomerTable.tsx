...
import CustomerAdjustBalanceModal from './CustomerAdjustBalanceModal'
...
export default function CustomerTable({
  ...
  const showActions = canUpdate || canDelete || canView || canAssignVoucher || canManageBalance || canViewWallet
  ...
  const handleBalanceAdjusted = (customerId: number, walletBalance: number) => {
    setRows((prev) =>
      prev.map((row) =>
        row.id === customerId ? { ...row, walletBalance } : row,
      ),
    )
    setToastMessage('Wallet balance updated successfully.')
    setTimeout(() => setToastMessage(null), 2500)
  }
    ...
    <div>
      ...
      {balanceTarget && (
        <CustomerAdjustBalanceModal
          customer={balanceTarget}
          canAdjust={canManageBalance}
          onClose={() => setBalanceTarget(null)}
          onSuccess={(walletBalance) => {
            handleBalanceAdjusted(balanceTarget.id, walletBalance)
