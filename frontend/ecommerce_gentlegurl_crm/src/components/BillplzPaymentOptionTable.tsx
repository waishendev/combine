...
export default function BillplzPaymentOptionTable({ permissions }: { permissions: string[] }) {
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      const url = isEditing
        ? `/api/proxy/ecommerce/billplz-payment-gateway-options/${editingId}?type=${payload.type}`
        : `/api/proxy/ecommerce/billplz-payment-gateway-options?type=${payload.type}`
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
        Demo codes may be seeded. Replace with real Billplz gateway/channel codes for production.
      </div>
      ...
      {(canCreate || (canUpdate && isEditing)) && (
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <input className="rounded border border-gray-300 px-3 py-2 text-sm" placeholder="Code" value={form.code} onChange={(e) => setForm((prev) => ({ ...prev, code: e.target.value }))} />
            <input className="rounded border border-gray-300 px-3 py-2 text-sm" placeholder="Name" value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} />
            <input className="rounded border border-gray-300 px-3 py-2 text-sm" placeholder="Logo URL (optional)" value={form.logo_url} onChange={(e) => setForm((prev) => ({ ...prev, logo_url: e.target.value }))} />
            <input className="rounded border border-gray-300 px-3 py-2 text-sm" placeholder="Sort Order" type="number" value={form.sort_order} onChange={(e) => setForm((prev) => ({ ...prev, sort_order: Number(e.target.value || 0) }))} />
            <select className="rounded border border-gray-300 bg-white px-3 py-2 text-sm" value={form.type} onChange={(e) => setForm((prev) => ({ ...prev, type: e.target.value as WorkspaceType }))}>
              <option value="ecommerce">Ecommerce</option>
              <option value="booking">Booking</option>
            </select>
            <select className="rounded border border-gray-300 bg-white px-3 py-2 text-sm" value={form.gateway_group} onChange={(e) => setForm((prev) => ({ ...prev, gateway_group: e.target.value as GatewayGroup }))}>
              <option value="online_banking">Online Banking</option>
              <option value="credit_card">Credit Card</option>
            </select>
          </div>
          <textarea className="mt-3 w-full rounded border border-gray-300 px-3 py-2 text-sm" placeholder="Description (optional)" value={form.description} onChange={(e) => setForm((prev) => ({ ...prev, description: e.target.value }))} />
          ...
          <div className="mt-4 flex gap-2">
            <button className="rounded bg-blue-600 px-4 py-2 text-sm font-medium text-white" onClick={submitForm}>
              {isEditing ? 'Update' : 'Create'}
            </button>
            {isEditing && (
              <button className="rounded border border-gray-300 px-4 py-2 text-sm" onClick={resetForm}>Cancel</button>
            )}
          </div>
        </div>
      )}
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td className="px-4 py-6 text-center text-gray-500" colSpan={canUpdate || canDelete ? 8 : 7}>Loading...</td></tr>
            ) : sortedRows.length === 0 ? (
              <tr><td className="px-4 py-6 text-center text-gray-500" colSpan={canUpdate || canDelete ? 8 : 7}>No options found.</td></tr>
            ) : (
              sortedRows.map((row) => (
