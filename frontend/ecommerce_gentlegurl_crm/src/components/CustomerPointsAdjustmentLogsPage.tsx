...
type PointsAdjustmentLogResponse = {
  rows?: PointsAdjustmentLogRow[]
...
const normalizeActionType = (value?: string | null) => {
  ...
  if (value === 'reduce_points') return 'reduce'
  return value ?? '—'
}

export default function CustomerPointsAdjustmentLogsPage() {
  const [rows, setRows] = useState<PointsAdjustmentLogRow[]>([])
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/proxy/customer-points-adjustment-logs?${qs.toString()}`, {
          cache: 'no-store',
        ...
        const payload: PointsAdjustmentLogResponse = response?.data ?? {}
        ...
        <table className="min-w-full border-collapse text-sm">
          <thead>
            <tr className="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wider text-slate-600">
              ...
              <th className="px-4 py-3 border border-gray-200">Points</th>
              <th className="px-4 py-3 border border-gray-200">Before Balance</th>
              <th className="px-4 py-3 border border-gray-200">After Balance</th>
          ...
          <tbody>
            {loading ? (
              Array.from({ length: 6 }).map((_, idx) => (
                <TableLoadingRow key={idx} colSpan={9} />
              ))
            ) : rows.length === 0 ? (
              <TableEmptyState colSpan={9} message="No member points adjustment logs found." />
            ) : (
              rows.map((row) => {
                const actionType = normalizeActionType(row.action_type)
                const actionTypeLabel = actionType === 'add' || actionType === 'reduce' ? actionType : '—'
                ...
                return (
                  <tr key={row.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 border border-gray-200">
                      <span
                        className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold capitalize ${actionTypeClass}`}
                      >
                        {actionTypeLabel}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 border border-gray-200">
                      {Number(row.before_balance || 0).toLocaleString()}
                    </td>
                    <td className="px-4 py-3 border border-gray-200">
                      {Number(row.after_balance || 0).toLocaleString()}
                    </td>
