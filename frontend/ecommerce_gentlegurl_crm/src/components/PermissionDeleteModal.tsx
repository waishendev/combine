...
export default function PermissionDeleteModal({
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/proxy/permissions/${permission.id}`, {
        method: 'DELETE',
        headers: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md mx-auto bg-white rounded-lg shadow-lg">
        ...
        <div className="px-5 py-4 space-y-4">
          <p className="text-sm text-gray-700">Are you sure you want to delete this permission?</p>
          <div className="rounded-md bg-yellow-100 px-4 py-3">
            <p className="text-sm font-semibold text-yellow-800">{permission.name}</p>
            <p className="text-xs text-yellow-800">{permission.slug}</p>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button
              ...
              disabled={submitting}
            >
              {submitting ? 'Deleting...' : t('admin.deleteAction')}
            </button>
          </div>
        </div>
      </div>
