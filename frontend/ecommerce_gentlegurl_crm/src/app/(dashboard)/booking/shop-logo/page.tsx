...
export default async function BookingShopLogoPage() {
  ...
  const canEdit = user.permissions.includes('booking.settings.update') || user.permissions.includes('booking.settings.view')
  ...
  return (
    <div className="overflow-y-auto py-6 px-10">
      <div className="text-xs mb-4 flex items-center text-gray-500">
        ...
        <span className="mx-1">/</span>
        <Link href="/booking/shop-logo" className="text-blue-600 hover:underline">
          Upload Logo
        </Link>
      </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-3xl font-semibold text-slate-900 leading-tight">Storefront Logo</h2>
          <p className="text-sm text-slate-500 mt-2 max-w-2xl">
            Update Booking storefront logo and favicon.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <LogoUploadForm
          canEdit={canEdit}
          title="Booking Shop Logo"
          description="Upload a clean, high-resolution logo for the booking storefront."
          logoKey="shop_logo_url"
          uploadEndpoint="/api/proxy/ecommerce/branding/shop-logo"
          forcedWorkspace="booking"
        />
