  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-semibold text-[var(--foreground)]">Redeem Rewards</h3>
            <p className="text-sm text-[var(--foreground)]/70">Use your points to claim vouchers, gifts, and more.</p>
          </div>
          <button
            type="button"
            onClick={() => {
        ...
        {loadingRewards ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 3 }).map((_, idx) => (
              <div key={idx} className="rounded-xl border border-[var(--muted)] bg-[var(--background)] p-4 shadow-sm">
                <div className="h-5 w-32 animate-pulse rounded bg-[var(--muted)]/60" />
                <div className="mt-3 h-4 w-24 animate-pulse rounded bg-[var(--muted)]/60" />
                <div className="mt-5 h-10 w-full animate-pulse rounded bg-[var(--muted)]/60" />
              </div>
            ))}
          </div>
        ) : rewards.length === 0 ? (
          <div className="rounded-lg border border-dashed border-[var(--muted)] bg-[var(--background)] px-5 py-8 text-center text-sm text-[var(--foreground)]/70">
            No rewards are available right now. Please check back later.
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {rewards.map((reward) => {
              const canRedeem = overview ? availablePoints >= reward.points_required : false;
              const isRedeeming = redeemingId === reward.id;
              return (
                <div
                  key={reward.id}
                  className="flex h-full flex-col justify-between rounded-xl border border-[var(--muted)] bg-[var(--background)] p-4 shadow-sm"
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-base font-semibold text-[var(--foreground)]">{reward.title}</p>
                    ...
                    {reward.description && (
                      <p className="text-sm text-[var(--foreground)]/70 line-clamp-3">{reward.description}</p>
                    )}
                  ...
                  <button
                    ...
                    className={`mt-4 inline-flex items-center justify-center rounded-full px-4 py-2 text-sm font-semibold transition ${
                      canRedeem && !isRedeeming
                        ...
                        : "cursor-not-allowed bg-[var(--muted)] text-[var(--foreground)]/60"
                    }`}
                  >
