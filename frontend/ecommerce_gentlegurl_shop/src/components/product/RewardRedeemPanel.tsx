  ...
  const handleRedeem = useCallback(async () => {
    ...
    try {
      ...
      setRedeemModal({
        ...
        description: productName ? `${productName} has been added to your cart.` : "Item added to your cart.",
        rewardType: "product",
        details: [
          ...(productName ? [{ label: "Reward", value: productName }] : []),
          ...(requiredPoints != null ? [{ label: "Points Used", value: `${requiredPoints.toLocaleString()} pts` }] : []),
        ],
      });
    } catch (error) {
  ...
  return (
    <div className="space-y-3 rounded-xl border border-pink-100 bg-pink-50/80 p-4">
      ...
      {remainingQuantity !== undefined && remainingQuantity !== null && (
        <div className="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-xs font-semibold text-gray-700">
          <span>Remaining</span>
          <span className="text-sm text-[#ec4899]">{remainingQuantity > 0 ? remainingQuantity : "Out of stock"}</span>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button
          ...
          disabled={redeeming || isOutOfStock || (isLoggedIn ? !hasEnoughPoints : false)}
          className={`inline-flex items-center justify-center rounded-md px-4 py-2 text-sm font-semibold transition ${
            (!isLoggedIn || hasEnoughPoints) && !isOutOfStock
        ...
        <RedeemModal
          ...
          actions={[
            redeemModal.status === "success"
              ? { label: "Confirm", onClick: () => setRedeemModal(null) }
