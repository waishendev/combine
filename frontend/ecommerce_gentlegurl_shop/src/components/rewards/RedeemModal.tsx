            ...
            {state.details && state.details.length > 0 && (
              <div className="mt-3 space-y-1 text-sm text-gray-700">
                {state.details.map((detail) => (
                  <div key={`${detail.label}-${detail.value}`} className="flex items-center justify-between gap-3 rounded-lg bg-gray-50 px-3 py-2">
                    <span className="text-xs font-semibold uppercase text-gray-500">{detail.label}</span>
