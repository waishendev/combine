const STORAGE_KEY = "booking_guest_token";
...
export function getOrCreateBookingGuestToken(): string {
  ...
  if (existingStorage || existingCookie) {
    ...
    if (!existingStorage && token) window.localStorage.setItem(STORAGE_KEY, token);
    if (!existingCookie && token) setCookie(STORAGE_KEY, token, ONE_YEAR_IN_SECONDS);
    return token;
  }
