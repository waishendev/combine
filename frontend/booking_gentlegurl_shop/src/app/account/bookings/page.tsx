      ...
      <div className="mt-6 space-y-3">
        {bookings.map((booking) => (
          <div key={booking.id} className="rounded-2xl border border-[var(--card-border)] bg-[var(--card)] p-5">
            ...
            <p className="text-sm text-[var(--text-muted)]">{booking.staff_name || "Any staff"}</p>
            <p className="text-sm text-[var(--text-muted)]">{new Date(booking.starts_at).toLocaleString("en-MY", { timeZone: process.env.NEXT_PUBLIC_TIMEZONE || "Asia/Kuala_Lumpur" })}</p>
            <p className="mt-1 text-sm">Status: {booking.status}</p>
            {booking.package_claim_status === 'reserved' ? (
              <p className="mt-1 text-sm text-[var(--status-warning)]">Package Applied (Reserved) — will be consumed when booking is completed.</p>
            ) : booking.package_claim_status === 'consumed' ? (
              <p className="mt-1 text-sm text-[var(--status-success)]">Consumed from package.</p>
            ) : booking.package_claim_status === 'released' ? (
              <p className="mt-1 text-sm text-[var(--text-muted)]">Package reservation released.</p>
            ) : null}
