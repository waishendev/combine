...
import { ApiError, forgotCustomerPassword, resendCustomerVerification } from "@/lib/apiClient";
...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.code === "EMAIL_NOT_VERIFIED") {
        setVerificationMessage(err.message ?? "Please verify your email before resetting your password.");
        return;
      }
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      setResendMessage(response.message ?? "We have resent a verification link.");
    } catch {
        ...
        <div className="relative mx-auto flex min-h-[70vh] max-w-6xl items-center justify-center px-4 py-14">
          <div className="w-full max-w-md">
            <div className="mb-6 text-center">
              ...
              <p className="mt-2 text-sm text-[var(--foreground)]/70">Enter your email to receive a reset link.</p>
            </div>
              ...
              {verificationMessage ? (
                <div className="mb-4 rounded-xl border border-[var(--status-error-border)] bg-[var(--status-error-bg)] px-3 py-2 text-sm text-[color:var(--status-error)]">
                  ...
                  <button
                    ...
                    disabled={resending}
                    className="mt-2 w-full rounded-lg border border-[var(--status-error-border)] px-3 py-2 text-xs font-medium text-[color:var(--status-error)] transition hover:border-[var(--error)] hover:text-[var(--accent-error)] disabled:cursor-not-allowed disabled:opacity-60"
                  >
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                <div className="space-y-1.5">
                  ...
                  <input
                    id="email"
                    type="email"
                    className="mt-2 w-full rounded-xl border border-[var(--input-border)] bg-[var(--input-bg)] px-3 py-2.5 text-sm text-[var(--foreground)] focus:border-[var(--accent)] focus:outline-none focus:ring-4 focus:ring-[var(--ring)]/25"
                    value={email}
