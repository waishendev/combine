<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration {
    public function up(): void
    {
        Schema::create('booking_service_categories', function (Blueprint $table) {
            $table->id();
            $table->string('name');
            $table->string('slug')->unique();
            $table->text('description')->nullable();
            $table->string('image_path')->nullable();
            $table->boolean('is_active')->default(true);
            $table->unsignedInteger('sort_order')->default(0);
            $table->timestamps();

            $table->index(['is_active', 'sort_order']);
        });

        Schema::create('booking_service_category_service', function (Blueprint $table) {
            $table->foreignId('booking_service_category_id')->constrained('booking_service_categories')->cascadeOnDelete();
            $table->foreignId('booking_service_id')->constrained('booking_services')->cascadeOnDelete();
            $table->timestamps();

            $table->primary(['booking_service_category_id', 'booking_service_id'], 'booking_service_category_service_primary');
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('booking_service_category_service');
        Schema::dropIfExists('booking_service_categories');
    }
};
