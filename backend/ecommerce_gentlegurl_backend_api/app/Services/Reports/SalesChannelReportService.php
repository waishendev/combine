...
class SalesChannelReportService
{
    ...
    private function aggregateBookingTotals($rows): array
    {
        return [
            'orders_count' => (int) $rows->count(),
            'booking_deposit_amount' => (float) $rows->where('type', 'deposit')->sum('net_amount'),
            'booking_settlement_amount' => (float) $rows->where('type', 'final_settlement')->sum('net_amount'),
            'addon_revenue' => (float) $rows->where('type', 'addon')->sum('net_amount'),
            'package_purchase_amount' => (float) $rows->where('type', 'package_purchase')->sum('net_amount'),
