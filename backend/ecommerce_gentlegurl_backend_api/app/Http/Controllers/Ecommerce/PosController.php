...
{
    /**
     * Request-local memo for appointmentSearch only (P1-A).
     * Cleared in begin/end; never static, Redis, or cross-request.
     */
    private bool $appointmentSearchMemoEnabled = false;

    /** @var array<string, mixed> */
    private array $appointmentSearchMemo = [];

    /**
     * P1-B: active order_items for candidate bookings, keyed by booking_id.
     * Null outside appointmentSearch preload window.
     *
    ...
    private const APPOINTMENT_SEARCH_INACTIVE_ORDER_STATUSES = [
        'voided',
        'cancelled',
        'draft',
    ];
    ...
    private function beginAppointmentSearchMemo(): void
    {
        $this->appointmentSearchMemoEnabled = true;
        $this->appointmentSearchMemo = [];
    }

    private function endAppointmentSearchMemo(): void
    {
     ...
     * @return T
     */
    private function rememberAppointmentSearch(string $key, callable $resolver): mixed
    {
        if (! $this->appointmentSearchMemoEnabled) {
            return $resolver();
        }

        if (array_key_exists($key, $this->appointmentSearchMemo)) {
            return $this->appointmentSearchMemo[$key];
        }

        return $this->appointmentSearchMemo[$key] = $resolver();
    }

    /**
     * Memoized wrapper around CustomerServicePackageService::resolvePosCartServiceItemIdsForBooking.
     *
     * @return list<int>
     */
    protected function resolvePosCartServiceItemIdsForBookingCached(int $bookingId): array
    {
        ...
        /** @var list<int> $ids */
        $ids = $this->rememberAppointmentSearch("pos_cart_ids:{$bookingId}", function () use ($bookingId) {
            return $this->customerServicePackageService->resolvePosCartServiceItemIdsForBooking($bookingId);
        });
    ...
    /**
     * Staff fallback used when booking_service_staff_splits has no rows.
     *
     * @return array{staff_id: int, staff_name: string, share_percent: int, service_commission_rate_snapshot: float}|null
     */
    protected function resolveStaffSplitFallbackStaff(int $staffId): ?array
    {
        $staffId = (int) $staffId;
        if ($staffId <= 0) {
            return null;
        ...
        /** @var array{staff_id: int, staff_name: string, share_percent: int, service_commission_rate_snapshot: float}|null $row */
        $row = $this->rememberAppointmentSearch("staff_fallback:{$staffId}", function () use ($staffId) {
            $fallbackStaff = Staff::query()->find($staffId, ['id', 'name', 'service_commission_rate']);
            if (! $fallbackStaff) {
                return null;
            }
    ...
    public function appointmentSearch(Request $request)
    {
        ...
        try {
            return $this->runAppointmentSearch($request);
        } finally {
    ...
    protected function runAppointmentSearch(Request $request)
    {
        ...
        // Sargable timestamp range (same calendar-day semantics as whereDate under app timezone).
        // Inclusive start-of-day .. exclusive next-day-start so DATE(start_at) bounds stay identical.
        // Bind wall-clock strings so timestamp-without-time-zone columns match prior whereDate results.
        if ($hasRange) {
    ...
    public function appointmentDetail(int $id)
    {
        ...
        return $this->respond([
            ...
            'receipts' => $history,
                ...
                'refund_transactions' => $this->resolveAppointmentRefundTransactions($booking),
            ...$this->resolveAppointmentVisitCheckoutMeta((int) $booking->id),
    ...
    public function eligiblePackagesForAppointment(Request $request, int $id)
    {
        ...
        $summary = $this->resolveAppointmentFinancialSummary($booking);
        $allLines = $this->resolveAppointmentPackageLinesForBooking($booking, $summary);
        $currentClaims = $this->resolveAppointmentPackageClaimsCollection($booking);

        return $this->respond($this->buildEligiblePackagesPayload((int) $booking->customer_id, $allLines, $currentClaims));
    }
    ...
    protected function resolveAppointmentPackageClaimsCollection(Booking $booking): \Illuminate\Support\Collection
    {
    ...
    protected function resolveAddonPackageLineBookingServiceId(array $addon, \Illuminate\Support\Collection $optionsById): int
    {
        ...
        return $option ? (int) ($option->linked_booking_service_id ?? 0) : 0;
    }

    /**
     * @return array<int, array{line_type: string, line_index: int, booking_service_id: int, service_name: string, cn_name?: string|null, parent_service_index?: int}>
     */
    protected function resolveAppointmentPackageLinesForBooking(Booking $booking, ?array $summary = null): array
    {
        $summary = $summary ?? $this->resolveAppointmentFinancialSummary($booking);
        $mainServices = collect($summary['main_services'] ?? []);
        $addonCandidates = collect($summary['addon_settlement_items'] ?? [])
            ->concat($summary['add_ons'] ?? [])
            ->concat($mainServices->flatMap(fn (array $service) => $service['add_ons'] ?? []))
            ->filter(fn ($addon) => is_array($addon))
            ->values();

        $optionIds = $addonCandidates
            ->filter(fn (array $addon) => (int) ($addon['linked_booking_service_id'] ?? 0) <= 0)
            ->map(fn (array $addon) => (int) ($addon['id'] ?? 0))
            ->filter(fn (int $id) => $id > 0)
            ->unique()
            ->values();

        $optionsById = $optionIds->isNotEmpty()
            ? BookingServiceQuestionOption::query()
                ->whereIn('id', $optionIds->all())
                ->get(['id', 'linked_booking_service_id', 'label', 'cn_label'])
                ->keyBy('id')
            : collect();
        ...
        $appendLine = function (array $line) use (&$allLines, &$seenKeys): void {
            $serviceId = (int) ($line['booking_service_id'] ?? 0);
            if ($serviceId <= 0) {
                return;
            }

            $key = (string) ($line['line_type'] ?? 'main_service')
                . ':'
                . (int) ($line['line_index'] ?? 0)
                . ':'
                . $serviceId;
        ...
        foreach ($mainServices as $idx => $service) {
            ...
            foreach (($service['add_ons'] ?? []) as $addonIdx => $addon) {
                ...
                $addonServiceId = $this->resolveAddonPackageLineBookingServiceId($addon, $optionsById);
                if ($addonServiceId <= 0) {
                    continue;
                }
                $appendLine([
                    ...
                    'booking_service_id' => $addonServiceId,
                    'service_name' => (string) ($addon['name'] ?? $addon['label'] ?? 'Add-on'),
                    'cn_name' => $addon['cn_name'] ?? $addon['cn_label'] ?? null,
                    'parent_service_index' => $idx,
                ]);
            }
        }

        foreach ((array) ($summary['addon_settlement_items'] ?? []) as $addonIdx => $addon) {
            if (! is_array($addon)) {
                continue;
            }
            $addonServiceId = $this->resolveAddonPackageLineBookingServiceId($addon, $optionsById);
        ...
        $listedAddonServiceIds = collect($allLines)
            ->filter(fn (array $line) => ($line['line_type'] ?? '') === 'addon')
            ->pluck('booking_service_id')
            ->map(fn ($id) => (int) $id)
            ->filter(fn (int $id) => $id > 0)
            ->unique();

        foreach ((array) ($summary['add_ons'] ?? []) as $addonIdx => $addon) {
            if (! is_array($addon)) {
                continue;
            }
            $addonServiceId = $this->resolveAddonPackageLineBookingServiceId($addon, $optionsById);
            if ($addonServiceId <= 0 || $listedAddonServiceIds->contains($addonServiceId)) {
                continue;
            }
            $appendLine([
                'line_type' => 'addon',
                'line_index' => 1000 + $addonIdx,
                'booking_service_id' => $addonServiceId,
                'service_name' => (string) ($addon['name'] ?? $addon['label'] ?? 'Add-on'),
                'cn_name' => $addon['cn_name'] ?? $addon['cn_label'] ?? null,
            ]);
    ...
    protected function buildEligiblePackagesPayload(int $customerId, array $allLines, $currentClaims): array
    {
        ...
        $currentClaimsMapped = $currentClaims->map(fn (CustomerServicePackageUsage $usage) => $this->mapPackageUsageClaimRow($usage))->values()->all();
        ...
        [$order, $receiptUrl] = DB::transaction(function () use ($request, $booking, $amount, $validated, $summary, $discountType, $discountValue, $discountRemark, $totalDiscount, $balanceDue, $paymentRows, $lineSplitPayloads, $staffCommissionRates) {
            ...
            $normalizeSplits = function ($splits) {
                return collect($splits ?? [])
                    ->map(fn ($split) => StaffSplitNormalizer::mapIncomingRow(is_array($split) ? $split : []))
            ...
            $findLineSplitPayload = function (string $lineKey) use ($lineSplitPayloads) {
                return StaffSplitNormalizer::findLineStaffSplitPayload($lineSplitPayloads, $lineKey);
            };
            ...
            $resolveLineSplits = function (string $lineKey, $lineSplits = [], $fallbackSplits = []) use ($findLineSplitPayload, $normalizeSplits, $booking) {
                ...
                if ($normalized->isEmpty() && (int) ($booking->staff_id ?? 0) > 0) {
                    $normalized = $normalizeSplits([[
            ...
            $persistLineSplits = function (OrderItem $orderItem, $splits, string $lineType, string $lineKey, float $amountBasis, array $snapshot = []) use ($staffCommissionRates): void {
                $splitInput = is_array($splits) ? $splits : collect($splits)->values()->all();
                if ($splitInput === []) {
                    return;
                }

                $normalized = app(StaffSplitNormalizer::class)->normalizeIncoming($splitInput, $amountBasis);
                if ($normalized['error']) {
                    abort(422, $normalized['error']);
                }
                $splitRows = collect($normalized['splits'] ?? [])->values();
                ...
                foreach ($splitRows as $split) {
                    OrderItemStaffSplit::query()->create([
                        ...
                        'share_amount' => StaffSplitNormalizer::persistedShareAmount($split),
                        'split_mode' => StaffSplitNormalizer::persistedSplitMode($split),
    ...
    public function finalizeAppointmentZeroSettlement(Request $request, int $id, OrderPaymentService $orderPaymentService)
    {
        ...
        if ($overpaidAmount > 0.0001 && ! (bool) ($summary['refund_handled'] ?? false)) {
            return $this->respondError(__('This booking has overpaid deposit. Please handle refund/credit in Edit Settlement before checkout.'), 422);
        }
    ...
    public function editAppointmentSettlement(Request $request, int $id)
    {
        ...
        if ($effectiveService && (int) ($booking->service_id ?? 0) !== (int) $effectiveService->id) {
            $this->customerServicePackageService->releaseActiveClaimsForBookingService(
                (int) $booking->id,
                (int) ($booking->service_id ?? 0),
            );
                ...
                ->map(function (array $item): array {
                    return [
                        ...
                        'staff_splits' => collect($item['staff_splits'] ?? [])->map(fn ($split) => [
                            ...
                            'share_amount' => array_key_exists('share_amount', $split) && $split['share_amount'] !== null
            ...
            $mainServiceRows = $serviceIds
                ->map(function (int $serviceId) use ($servicesById, $existingMainByServiceId, $itemsPayload) {
                    ...
                    return [
                        ...
                        'staff_splits' => $itemSplits->map(fn ($split) => [
                            'staff_id' => (int) ($split['staff_id'] ?? 0),
                            'share_percent' => (int) ($split['share_percent'] ?? 0),
                            'share_amount' => array_key_exists('share_amount', $split) && $split['share_amount'] !== null
                                ? round((float) $split['share_amount'], 2)
        ...
        $this->customerServicePackageService->releaseOrphanedPackageClaimsForBooking(
            (int) $booking->id,
            $this->resolveAppointmentPackageServiceIds($booking)->all(),
        );

        $summary = $this->resolveAppointmentFinancialSummary($booking);
        $originalMainService = collect($summary['main_services'] ?? [])
            ->first(fn (array $service, int $idx) => ($service['is_original'] ?? false) || $idx === 0);
        $bookingLineTotal = round(max(0, (float) ($originalMainService['extra_price'] ?? 0)), 2);
        if ($bookingLineTotal <= 0) {
            $bookingLineTotal = round(max(0, (float) ($summary['service_total'] ?? 0)), 2);
        }
        if ($bookingLineTotal <= 0 && $booking->service) {
            $bookingLineTotal = round(max(0, (float) ($booking->service->service_price ?? $booking->service->price ?? 0)), 2);
        }
    ...
    public function addAppointmentRefund(Request $request, int $id)
    {
        ...
        $summary = $this->resolveAppointmentFinancialSummary($booking);
        $refundNeeded = round(max(0.0, (float) ($summary['refund_needed'] ?? 0)), 2);
        $amount = round((float) $validated['amount'], 2);

        if ($refundNeeded <= 0.0001) {
            return $this->respondError(__('No overpaid amount requires refund or customer credit.'), 422);
        }
        if ($amount > $refundNeeded + 0.0001) {
    ...
    public function editAppointmentRefundTransaction(Request $request, int $id, int $refundId)
    {
        ...
        $summary = $this->resolveAppointmentFinancialSummary($booking);
        $overpaidAmount = round(max(0.0, (float) ($summary['overpaid_amount'] ?? 0)), 2);
        $otherRefunded = round((float) BookingRefund::query()
            ->where('booking_id', (int) $booking->id)
            ->where('status', 'completed')
            ->where('id', '!=', (int) $refund->id)
            ->sum('amount'), 2);
        $maxAmount = round(max(0.0, $overpaidAmount - $otherRefunded), 2);
        $amount = round((float) $validated['amount'], 2);
    ...
    public function voidAppointmentRefund(Request $request, int $id)
    {
        $validated = $request->validate([
        ...
        $existingRemark = trim((string) ($refund->remark ?? ''));
        ...
        if (! $booking) {
            return $this->respond(['refund_id' => (int) $refund->id], __('Refund voided.'));
        }
    ...
    public function refundReceipt(int $id)
    {
        ...
        return $this->respond([
            'refund' => [
                ...
                'receipt_public_url' => $this->buildReceiptUrlForRefund((int) $refund->id),
            ],
        ]);
    }
    ...
    public function addService(Request $request)
    {
        ...
        $serviceLineTotal = round(max(0, (float) ($service->price ?? $service->service_price ?? 0) + $addonPrice), 2);
        $normalizedResult = app(StaffSplitNormalizer::class)->normalize($splits->all(), $serviceLineTotal);
        if ($normalizedResult['error']) {
            return $this->respondError((string) $normalizedResult['error'], 422);
        }
        ...
        $normalizedSplits = collect($normalizedResult['splits'])->map(fn (array $split) => [
            'staff_id' => (int) $split['staff_id'],
            'share_percent' => (int) $split['share_percent'],
    ...
    public function createAppointment(Request $request, OrderPaymentService $orderPaymentService)
    {
        ...
        $bookingLineTotal = round(max(0, (float) ($service->service_price ?? $service->price ?? 0) + $addonPrice), 2);
        $normalizedResult = app(StaffSplitNormalizer::class)->normalize($splits->all(), $bookingLineTotal);
        if ($normalizedResult['error']) {
            return $this->respondError((string) $normalizedResult['error'], 422);
        }
        ...
        [$booking, $depositOrder, $depositReceiptUrl] = DB::transaction(function () use (
            ...
            if ($depositAmount > 0) {
                ...
                $persistDepositLineSplits = function (OrderItem $orderItem, array $splits, float $amountBasis, string $lineRefId) use ($staffCommissionRates): void {
                    $normalized = app(StaffSplitNormalizer::class)->normalizeIncoming($splits, $amountBasis);
                    if ($normalized['error']) {
                        abort(422, $normalized['error']);
                    }
                    $rows = collect($normalized['splits'] ?? [])
                        ->map(fn (array $split) => [
                            'order_item_id' => (int) $orderItem->id,
                            'line_type' => 'booking_deposit',
                            ...
                            'split_mode' => StaffSplitNormalizer::persistedSplitMode($split),
                            'amount_basis' => round(max(0, $amountBasis), 2),
                            'commission_rate_snapshot' => (float) ($staffCommissionRates[(int) ($split['staff_id'] ?? 0)] ?? 0),
    ...
    public function checkout(Request $request, OrderPaymentService $orderPaymentService)
    {
        ...
        [$order, $receiptUrl, $purchasedPackageLines, $confirmedBookingIds, $bookingRefunds, $checkedOutBookings] = DB::transaction(function () use ($validated, $cart, $request, $orderPaymentService) {
            ...
            $staffIds = $staffSplitsByCartItemId
                ...
                ->merge(collect($validated['package_items'] ?? [])->flatMap(fn (array $item) => collect($item['staff_splits'] ?? [])->pluck('staff_id')))
                ->filter()
                ->map(fn ($staffId) => (int) $staffId)
                ->unique()
                ->values();
            ...
            $persistOrderItemLineSplits = function (OrderItem $orderItem, $splits, string $lineType, ?string $lineRefId, float $amountBasis, array $snapshot = []) use ($staffCommissionRates): void {
                $normalized = app(StaffSplitNormalizer::class)->normalizeIncoming(is_array($splits) ? $splits : [], $amountBasis);
                if ($normalized['error']) {
                    abort(422, $normalized['error']);
                }
                $splitRows = collect($normalized['splits'] ?? [])->values();
            ...
            $findLineSplitPayload = fn ($linePayloads, string $lineKey) => StaffSplitNormalizer::findLineStaffSplitPayload($linePayloads ?? [], $lineKey);
            ...
            foreach ($cart->items as $item) {
                ...
                if ($itemType === 'BOOKING_PRODUCT' || $itemType === 'booking_product') {
                    ...
                    if ($itemSplits->isNotEmpty()) {
                        ...
                        $persistOrderItemLineSplits(
                            $orderItem,
                            $itemSplits->values()->all(),
                            'booking_product_base',
                            (string) $item->id,
                            (float) ($baseCommissionLine['amount_basis'] ?? 0),
                            [
                    ...
                    foreach ($bookingProductCommissionLines as $commissionLine) {
                        ...
                        $persistOrderItemLineSplits(
                            $orderItem,
                            $splits->all(),
                            'booking_product_option',
                            $optionId,
                            (float) ($commissionLine['amount_basis'] ?? 0),
                            [
                                'cart_item_id' => (int) $item->id,
                ...
                if ($itemSplits->isNotEmpty()) {
                    $lineNetTotal = round((float) ($pricing['line_total_after_discount'] ?? $pricing['effective_line_total'] ?? 0), 2);
                    $persistOrderItemLineSplits($orderItem, $itemSplits->values()->all(), 'product', null, $lineNetTotal);
                }
            }
            ...
            foreach ($cart->serviceItems as $serviceItem) {
                ...
                $mainPayloadSplits = $resolveLineSplits($serviceLinePayloads, 'main', collect($serviceItem->staff_splits ?? [])->values()->all());
                $bookingSplitBasis = round(max(0, (float) ($depositByServiceItemId[(int) $serviceItem->id] ?? 0)), 2);
                if ($bookingSplitBasis <= 0) {
                    $bookingSplitBasis = $lineTotal;
                }
                $normalizedBookingSplits = app(StaffSplitNormalizer::class)->normalizeIncoming($mainPayloadSplits, $bookingSplitBasis);
                if ($normalizedBookingSplits['error']) {
                    abort(422, $normalizedBookingSplits['error']);
                }
                $splits = collect($normalizedBookingSplits['splits'] ?? [])->map(function (array $split) use ($staffCommissionRates, $serviceItem) {
                    return [
                        'staff_id' => (int) ($split['staff_id'] ?? 0),
                        'share_percent' => (int) ($split['share_percent'] ?? 0),
                        'share_amount' => $split['share_amount'] ?? null,
                        'service_commission_rate_snapshot' => (float) ($staffCommissionRates[(int) ($split['staff_id'] ?? 0)] ?? $serviceItem->commission_rate_used ?? 0),
                    ];
                });
                if ($splits->isEmpty()) {
                    $splits = collect([[
                        ...
                        'share_percent' => 100,
                        'share_amount' => $bookingSplitBasis > 0 ? $bookingSplitBasis : null,
            ...
            foreach ($cart->packageItems as $packageItem) {
                ...
                $normalizedPackageSplits = app(StaffSplitNormalizer::class)->normalizeIncoming(
                    $splitRows->values()->all(),
                    round($packageLineNet, 2),
                );
                if ($normalizedPackageSplits['error']) {
                    abort(422, $normalizedPackageSplits['error']);
                }
                $normalizedSplits = collect($normalizedPackageSplits['splits'] ?? [])->values();
                $packageQty = max(1, (int) $packageItem->qty);
                for ($i = 0; $i < $packageQty; $i++) {
                    ...
                    foreach ($normalizedSplits as $split) {
                        ...
                        $splitSales = isset($split['share_amount']) && $split['share_amount'] !== null
                            ? round(((float) $split['share_amount']) / $packageQty, 2)
                            : round($unitNetAmount * ($sharePercent / 100), 2);
                        $rate = (float) ($staffCommissionRates[$staffId] ?? 0);
            ...
            foreach ($cart->appointmentSettlementItems as $settlementItem) {
                ...
                if ($serviceBalanceDue > 0.0001) {
                    ...
                    foreach ($serviceLines as $mainLine) {
                        ...
                        $lineSplits = collect($mainLine['staff_splits'] ?? [])
                            ->map(fn ($split) => StaffSplitNormalizer::mapIncomingRow(is_array($split) ? $split : []))
                            ->filter(fn (array $split) => $split['staff_id'] > 0 && ($split['share_percent'] > 0 || ($split['share_amount'] ?? 0) > 0))
                            ->values();
    ...
    protected function respondAppointmentRefundMutation(Booking $booking, string $message)
    {
        $summary = $this->resolveAppointmentFinancialSummary($booking);
    ...
    protected function resolveAppointmentRefundTransactions(Booking $booking): array
    {
        $methodLabels = [
            'cash' => 'Cash Refund',
        ...
        return BookingRefund::query()
            ...
            ->map(function (BookingRefund $refund) use ($methodLabels) {
                ...
                return [
                    ...
                    'receipt_public_url' => $this->buildReceiptUrlForRefund((int) $refund->id),
                    'created_by' => $refund->processor ? [
                        'id' => (int) $refund->processor->id,
                        'name' => (string) $refund->processor->name,
                    ] : null,
    ...
    protected function buildReceiptUrlForRefund(int $refundId): ?string
    {
    ...
    protected function activeBookingOrderItemQuery(?int $bookingId = null): \Illuminate\Database\Eloquent\Builder
    {
        $query = OrderItem::query()
            ->whereHas('order', fn ($orderQuery) => $orderQuery->whereNotIn('status', self::APPOINTMENT_SEARCH_INACTIVE_ORDER_STATUSES));
    ...
    /**
     * P1-B: one query for all candidate booking IDs used by appointmentSearch financial summary.
     *
     * Replaces these per-booking activeBookingOrderItemQuery calls inside resolveAppointmentFinancialSummary:
     * 1) line_type=booking_addon (addon paid + addon settlement filter on variant)
     * 2) line_type=booking_deposit (main deposit collected)
     * 3) line_type=booking_addon + variant_name_snapshot='Booking Add-on Deposit'
     * 4) line_type=booking_settlement (main settlement paid)
     *
     * Shared conditions preserved:
     * - order.status NOT IN (voided, cancelled, draft) via whereHas('order')
     * - booking_id in candidate set
     * - only the line_types listed above (exact set used by those four call sites)
     *
     * @param  list<int>  $bookingIds
     */
    protected function preloadAppointmentSearchActiveOrderItems(array $bookingIds): void
    {
        ...
        if ($bookingIds === []) {
            $this->appointmentSearchActiveOrderItemsByBookingId = collect();
        ...
        $items = OrderItem::query()
            ...
            ->whereIn('line_type', self::APPOINTMENT_SEARCH_ACTIVE_ORDER_LINE_TYPES)
            ->whereHas(
                'order',
                fn ($orderQuery) => $orderQuery->whereNotIn('status', self::APPOINTMENT_SEARCH_INACTIVE_ORDER_STATUSES)
            )
            ->get([
                'id',
                'order_id',
        ...
        $this->appointmentSearchActiveOrderItemsByBookingId = $items
     ...
     */
    protected function activeBookingOrderItemsFiltered(
        int $bookingId,
        string $lineType,
        ...
        if ($this->appointmentSearchActiveOrderItemsByBookingId !== null) {
            return $this->appointmentSearchActiveOrderItemsByBookingId
                ->get($bookingId, collect())
                ->filter(function (OrderItem $item) use ($lineType, $variantNameSnapshot) {
                    if ((string) ($item->line_type ?? '') !== $lineType) {
                        return false;
                    }
                    if ($variantNameSnapshot !== null
                        && (string) ($item->variant_name_snapshot ?? '') !== $variantNameSnapshot) {
                        return false;
                    }
        ...
        $query = $this->activeBookingOrderItemQuery($bookingId)->where('line_type', $lineType);
        if ($variantNameSnapshot !== null) {
            $query->where('variant_name_snapshot', $variantNameSnapshot);
        }
    ...
    protected function serializeCart(PosCart $cart): array
    {
        ...
        $settlementItems = $cart->appointmentSettlementItems->map(function (PosCartAppointmentSettlementItem $item) {
            ...
            $packageClaims = $this->resolvePerLinePackageClaims($booking, $summary);
            $claimedServiceIds = collect($packageClaims)
                ->pluck('booking_service_id')
                ->map(fn ($id) => (int) $id)
                ->filter(fn (int $id) => $id > 0)
                ->unique()
                ->values();
            $depositContribution = max(0.0, (float) ($summary['deposit_contribution'] ?? 0));
            $legacyPackageOffset = max(0.0, (float) ($summary['package_offset'] ?? 0));
            $rawMainSettlementItems = collect((array) ($summary['main_service_settlement_items'] ?? []));
            $originalMainLine = $rawMainSettlementItems->first(
                fn (array $line, int $idx) => (bool) ($line['is_original'] ?? ($idx === 0))
            );
            $originalMainServiceId = (int) (
                (is_array($originalMainLine) ? ($originalMainLine['linked_booking_service_id'] ?? null) : null)
                ?: ($booking->service_id ?? 0)
            );
            $mainSettlementItems = $rawMainSettlementItems->map(function (array $line) use (
                $item,
                $claimedServiceIds,
                $legacyPackageOffset,
                $depositContribution,
                ...
                $serviceId = (int) ($line['linked_booking_service_id'] ?? $line['id'] ?? 0);
                $linePackageOffset = 0.0;
                if ($claimedServiceIds->isNotEmpty()) {
                    $linePackageOffset = $claimedServiceIds->contains($serviceId) ? $gross : 0.0;
                } elseif ($isOriginal) {
                    $linePackageOffset = $legacyPackageOffset;
                }
                $mainLineCovered = $gross > 0.0001 && $linePackageOffset >= $gross - 0.0001;
                $depositCredit = ($isOriginal && ! $mainLineCovered) ? $depositContribution : 0.0;
                $balanceDue = max(0.0, $gross - $paidAmount - $depositCredit - $linePackageOffset);
            ...
            $originalMainCovered = $claimedServiceIds->contains($originalMainServiceId)
                || ($claimedServiceIds->isEmpty() && $legacyPackageOffset > 0.0001);
            $addonDepositCreditRemaining = $originalMainCovered ? $depositContribution : 0.0;
            $addonSettlementItems = collect((array) ($summary['addon_settlement_items'] ?? []))->map(function (array $line) use ($item, &$addonDepositCreditRemaining, $claimedServiceIds) {
                ...
                $summaryGross = max(0.0, (float) ($line['gross_amount'] ?? 0));
                $summaryBalanceDue = max(0.0, (float) ($line['balance_due'] ?? 0));
                $paidAmount = max(0.0, (float) ($line['paid_amount'] ?? 0));
                $addonServiceId = (int) ($line['linked_booking_service_id'] ?? 0);
                $linePackageOffset = 0.0;
                if ($claimedServiceIds->isNotEmpty() && $addonServiceId > 0 && $claimedServiceIds->contains($addonServiceId)) {
                    $linePackageOffset = $gross;
                }
                $lineCoveredByPackage = $gross > 0.0001 && $linePackageOffset >= $gross - 0.0001;
                $depositCredit = 0.0;
                if (! $lineCoveredByPackage && $addonDepositCreditRemaining > 0.0001 && $paidAmount <= 0.0001) {
                    $depositCredit = min($addonDepositCreditRemaining, $gross);
                    $addonDepositCreditRemaining = max(0.0, round($addonDepositCreditRemaining - $depositCredit, 2));
                }
                $balanceDue = max(0.0, $gross - $paidAmount - $depositCredit - $linePackageOffset);
                if ($linePackageOffset <= 0.0001 && $depositCredit <= 0.0001 && abs($gross - $summaryGross) < 0.01) {
                    $balanceDue = $summaryBalanceDue;
                }
                $discount = $this->resolveAppointmentSettlementLineDiscount($item, $lineKey, $balanceDue);
            ...
            if ($originalMainCovered && $addonDepositCreditRemaining > 0.0001) {
                $mainSettlementItems = $mainSettlementItems->map(function (array $line) use (
                    $item,
                    &$addonDepositCreditRemaining,
                    $claimedServiceIds,
                ) {
                    if (($line['is_original'] ?? false) === true || $addonDepositCreditRemaining <= 0.0001) {
                        return $line;
                    }

                    $serviceId = (int) ($line['linked_booking_service_id'] ?? $line['id'] ?? 0);
                    $gross = max(0.0, (float) ($line['gross_amount'] ?? $line['extra_price'] ?? 0));
                    if ($serviceId > 0 && $claimedServiceIds->contains($serviceId) && $gross > 0.0001) {
                        return $line;
                    }

                    $currentDue = max(0.0, (float) ($line['balance_due'] ?? 0));
                    if ($currentDue <= 0.0001) {
                        return $line;
                    }

                    $depositCredit = min($addonDepositCreditRemaining, $currentDue);
                    $addonDepositCreditRemaining = max(0.0, round($addonDepositCreditRemaining - $depositCredit, 2));
                    $newDue = max(0.0, round($currentDue - $depositCredit, 2));
                    $lineKey = (string) ($line['line_key'] ?? '');
                    $discount = $this->resolveAppointmentSettlementLineDiscount($item, $lineKey, $newDue);

                    return [
                        ...$line,
                        'balance_due' => round($newDue, 2),
                        'discount_type' => $discount['discount_type'],
                        'discount_value' => $discount['discount_value'],
                        'discount_amount' => $discount['discount_amount'],
                        'discount_remark' => $discount['discount_remark'],
    ...
    protected function resolvePackageRedemptionUnitsForBooking(int $bookingId): array
    {
        ...
        $posCartItemIds = $this->resolvePosCartServiceItemIdsForBookingCached($bookingId);
    ...
    protected function resolveBookingProductCommissionLines(int $cartItemId, int $qty, float $lineNetTotal, $optionSnapshots, $parentSplits = [], $lineSplitPayloads = []): array
    {
        ...
        $findLineSplitPayload = function (string $lineKey, string $fallbackKey = '') use ($lineSplitPayloads) {
            return StaffSplitNormalizer::findLineStaffSplitPayload($lineSplitPayloads ?? [], $lineKey)
                ?? ($fallbackKey !== '' ? StaffSplitNormalizer::findLineStaffSplitPayload($lineSplitPayloads ?? [], $fallbackKey) : null);
        };
    ...
    {
        // $summary is unused by claim resolution today; key covers booking fields that affect filtering.
        $bookingId = (int) $booking->id;
        $serviceId = (int) ($booking->service_id ?? 0);
        $addonFingerprint = md5(json_encode($booking->addon_items_json ?? []));
        $memoKey = "pkg_claims:{$bookingId}:svc:{$serviceId}:addons:{$addonFingerprint}";

        /** @var list<array<string, mixed>> $claims */
        $claims = $this->rememberAppointmentSearch($memoKey, function () use ($booking) {
            return $this->computePerLinePackageClaims($booking);
        });
     ...
     */
    protected function computePerLinePackageClaims(Booking $booking): array
    {
        $posCartItemIds = $this->resolvePosCartServiceItemIdsForBookingCached((int) $booking->id);
        ...
        $claims = $claims->filter(function (CustomerServicePackageUsage $usage) use ($activeServiceIds) {
        ...
        return $claims->map(fn (CustomerServicePackageUsage $usage) => $this->mapPackageUsageClaimRow($usage))->values()->all();
    }

    protected function mapPackageUsageClaimRow(CustomerServicePackageUsage $usage): array
    {
        return [
            ...
            'used_from' => (string) ($usage->used_from ?? 'ADMIN'),
        ];
    }
    ...
    protected function resolveSettlementLineRangeBoundMin(array $item): float
    {
        $quantity = max(1, (int) ($item['quantity'] ?? 1));
        $mode = (string) ($item['price_mode'] ?? $item['linked_price_mode'] ?? 'fixed');
        $finalized = (bool) ($item['price_finalized'] ?? false);

        if ($mode === 'range' && ! $finalized) {
            $minRaw = $item['price_range_min'] ?? $item['linked_price_range_min'] ?? null;
            $maxRaw = $item['price_range_max'] ?? $item['linked_price_range_max'] ?? null;
            if ($minRaw !== null || $maxRaw !== null) {
                $min = round(max(0, (float) ($minRaw ?? $maxRaw ?? 0)), 2);
                $max = round(max(0, (float) ($maxRaw ?? $minRaw ?? 0)), 2);

                return round(min($min, $max) * $quantity, 2);
            }
        }

        if (array_key_exists('line_gross_amount', $item) && $item['line_gross_amount'] !== null) {
            return round(max(0, (float) $item['line_gross_amount']), 2);
        }

        return round(max(0, (float) ($item['extra_price'] ?? 0)) * $quantity, 2);
    }
    ...
    protected function resolveAppointmentFinancialSummary(Booking $booking): array
    {
            ...
            ->map(fn ($item) => [
                ...
                'staff_splits' => collect($item['staff_splits'] ?? [])->map(fn ($split) => [
                    ...
                    'share_amount' => array_key_exists('share_amount', $split) && $split['share_amount'] !== null
                        ? round((float) $split['share_amount'], 2)
                        : null,
                    'split_mode' => StaffSplitNormalizer::persistedSplitMode(is_array($split) ? $split : []),
                ])->filter(fn ($split) => $split['staff_id'] > 0 && ($split['share_percent'] > 0 || ($split['share_amount'] ?? 0) > 0))->values()->all(),
            ])
            ->values();
        $mainServices = collect([[
        ...
        $addonPaidRows = $this->activeBookingOrderItemsFiltered((int) $booking->id, 'booking_addon');
        ...
        $mainDepositCollected = round((float) $this->activeBookingOrderItemsFiltered((int) $booking->id, 'booking_deposit')
            ->sum(fn (OrderItem $row) => $this->invoiceService->resolveOrderItemCollectedAmount($row)), 2);
        $addonDepositCollected = round((float) $this->activeBookingOrderItemsFiltered(
            (int) $booking->id,
            'booking_addon',
        ...
        $mainSettlementPaidRows = $this->activeBookingOrderItemsFiltered((int) $booking->id, 'booking_settlement');
        ...
        $hasUnsettledRangePricing = ($isMainRangePriced && ! $originalServicePriceFinalized)
            || $addonItems->contains(function (array $addon): bool {
                $mode = (string) ($addon['price_mode'] ?? $addon['linked_price_mode'] ?? 'fixed');

                return $mode === 'range' && ! ($addon['price_finalized'] ?? false);
            });
        $totalOutstanding = round($serviceBalanceDue + $addonBalanceDue, 2);
        $rangeAwareGrossMin = round(
            (float) $mainServices->sum(fn (array $main) => $this->resolveSettlementLineRangeBoundMin($main))
            + (float) $addonItems->sum(fn (array $addon) => $this->resolveSettlementLineRangeBoundMin($addon)),
            2
        );
        $rangeAwarePackageCoveredMin = 0.0;
        if ($claimedServiceIds->isNotEmpty()) {
            $mainServiceIds = $mainServices
                ->map(fn (array $service) => (int) ($service['linked_booking_service_id'] ?? $service['id'] ?? 0))
                ->filter(fn (int $id) => $id > 0)
                ->unique()
                ->values();
            $rangeAwarePackageCoveredMin = round(
                (float) $mainServices
                    ->filter(fn (array $service) => $claimedServiceIds->contains((int) ($service['linked_booking_service_id'] ?? $service['id'] ?? 0)))
                    ->sum(fn (array $service) => $this->resolveSettlementLineRangeBoundMin($service))
                + (float) $addonItems
                    ->filter(function (array $addon) use ($claimedServiceIds, $mainServiceIds) {
                        $addonServiceId = (int) ($addon['linked_booking_service_id'] ?? 0);
                        if ($addonServiceId <= 0 || $mainServiceIds->contains($addonServiceId)) {
                    ...
                    ->sum(fn (array $addon) => $this->resolveSettlementLineRangeBoundMin($addon)),
                2
            );
        } elseif ($coveredByPackage) {
            $originalMain = $mainServices->first(fn (array $service) => ($service['is_original'] ?? false) === true);
            $rangeAwarePackageCoveredMin = $originalMain
                ? $this->resolveSettlementLineRangeBoundMin($originalMain)
                : 0.0;
        }
        $netOwedAtRangeMin = max(0.0, round($rangeAwareGrossMin - $rangeAwarePackageCoveredMin, 2));
        $netPaidTowardBalance = round($depositPaid + $settlementPaid, 2);
        if ($hasUnsettledRangePricing) {
            // Mirror settlement UI: compare collected cash against range-min obligation after package credits.
            $overpaidAmount = $netOwedAtRangeMin > $netPaidTowardBalance + 0.0001
                ? 0.0
                : max(0.0, round($netPaidTowardBalance - $netOwedAtRangeMin, 2));
        } elseif ($totalOutstanding > 0.0001) {
            $overpaidAmount = 0.0;
        } else {
        ...
        return [
            ...
            'has_pending_range_pricing' => $hasUnsettledRangePricing,
    ...
    protected function resolveAppointmentPackageServiceIds(Booking $booking): \Illuminate\Support\Collection
    {
        $ids = $this->resolveAppointmentMainServiceIds($booking);

        foreach ((array) ($booking->addon_items_json ?? []) as $item) {
            $itemKind = strtolower((string) ($item['item_kind'] ?? 'addon'));
            if ($itemKind === 'main_service') {
                foreach ((array) ($item['addon_items'] ?? []) as $addon) {
                    $linkedId = (int) (is_array($addon) ? ($addon['linked_booking_service_id'] ?? 0) : 0);
                    if ($linkedId > 0) {
                        $ids->push($linkedId);
                    }
                }
    ...
    {
        $bookingId = (int) $bookingId;
        $fallbackStaffId = (int) $fallbackStaffId;
        $memoKey = "staff_splits:{$bookingId}:fallback:{$fallbackStaffId}";

        /** @var list<array<string, mixed>> $rows */
        $rows = $this->rememberAppointmentSearch($memoKey, function () use ($bookingId, $fallbackStaffId) {
            return $this->computeBookingStaffSplits($bookingId, $fallbackStaffId);
        });

        // Return a fresh collection so callers cannot mutate the memoized list.
        return collect($rows);
    }
     ...
     * @return list<array<string, mixed>>
     */
    protected function computeBookingStaffSplits(int $bookingId, int $fallbackStaffId = 0): array
    {
        ...
        $fallback = $this->resolveStaffSplitFallbackStaff($fallbackStaffId);
        if ($fallback === null) {
            return [];
        }
    ...
    protected function normalizeBookingStaffSplits($splits, int $fallbackStaffId, float $lineTotal = 0): array
    {
        ...
        $incoming = $rows->map(fn ($row) => StaffSplitNormalizer::mapIncomingRow(is_array($row) ? $row : []))->all();
        $explicitMode = collect($incoming)->pluck('split_mode')->filter()->first();
        $normalized = app(StaffSplitNormalizer::class)->normalize(
            $incoming,
            round(max(0, $lineTotal), 2),
            $explicitMode === StaffSplitNormalizer::MODE_AMOUNT || $explicitMode === StaffSplitNormalizer::MODE_PERCENT
                ? (string) $explicitMode
                : (collect($incoming)->contains(fn (array $row) => ($row['share_amount'] ?? 0) > 0)
                    ? StaffSplitNormalizer::MODE_AMOUNT
                    : null),
        );
        if ($normalized['error']) {
            return ['error' => $normalized['error'], 'splits' => []];
        }

        $uniqueStaff = collect($normalized['splits'])->pluck('staff_id')->unique();
        $staffRates = Staff::query()
            ->whereIn('id', $uniqueStaff->all())
            ->pluck('service_commission_rate', 'id');

        $withRate = collect($normalized['splits'])->map(fn ($row) => [
            'staff_id' => (int) $row['staff_id'],
            'share_percent' => (int) $row['share_percent'],
            'share_amount' => $row['share_amount'] ?? null,
            'split_mode' => (string) ($row['split_mode'] ?? StaffSplitNormalizer::MODE_PERCENT),
    ...
    protected function persistBookingStaffSplits(Booking $booking, $normalizedSplits): void
    {
        ...
        $rows = collect($normalizedSplits)->map(fn ($split) => [
            ...
            'created_at' => now(),
            'updated_at' => now(),
        ])->filter(fn ($row) => $row['staff_id'] > 0 && ($row['split_percent'] > 0 || ($row['share_amount'] ?? 0) > 0))->values()->all();
    ...
    protected function recordPackageCoveredAppointmentOnOrder(Order $order, Booking $booking): void
    {
        ...
        $staffCommissionRates = DB::table('staffs')
            ->pluck('service_commission_rate', 'id')
            ->mapWithKeys(fn ($rate, $id) => [(int) $id => (float) $rate])
            ->all();
        $packageRedemptionUnitsByService = $this->resolvePackageRedemptionUnitsForBooking((int) $booking->id);
        $takePackageRedemptionValue = function (int $bookingServiceId, int $quantity = 1) use (&$packageRedemptionUnitsByService): float {
            $quantity = max(1, $quantity);
        ...
        $bookingSplits = $this->resolveBookingStaffSplits((int) $booking->id, (int) ($booking->staff_id ?? 0))->values()->all();
        $splitNormalizer = app(StaffSplitNormalizer::class);
        $persistPackageRedemptionSplits = function (
            OrderItem $orderItem,
            array $splits,
            string $lineKey,
            float $redemptionValue,
            array $snapshot
        ) use ($staffCommissionRates, $splitNormalizer): void {
            if ($redemptionValue <= 0.0001 || $splits === []) {
                return;
            }

            $normalized = $splitNormalizer->normalizeIncoming($splits, $redemptionValue);
            if (! empty($normalized['error']) || empty($normalized['splits'])) {
                return;
            }

            foreach ($normalized['splits'] as $split) {
                OrderItemStaffSplit::query()->create([
                    'order_item_id' => (int) $orderItem->id,
                    'line_type' => 'settlement_package_redemption',
                    'line_ref_id' => $lineKey,
                    ...
                    'share_amount' => StaffSplitNormalizer::persistedShareAmount($split),
                    'split_mode' => StaffSplitNormalizer::persistedSplitMode($split),
                    'amount_basis' => round(max(0, $redemptionValue), 2),
                    'commission_rate_snapshot' => (float) ($staffCommissionRates[(int) $split['staff_id']] ?? 0),
                    'snapshot' => $snapshot,
                ]);
            }
        ...
        foreach ($serviceLines as $mainIdx => $mainLine) {
            ...
            $serviceName = (string) ($mainLine['name'] ?? $booking->service?->name ?? 'Service');
            $bookingServiceId = (int) ($mainLine['linked_booking_service_id'] ?? $booking->service_id ?? 0);
            $serviceOrderItem = OrderItem::query()->create([
                'order_id' => (int) $order->id,
                'line_type' => 'booking_settlement',
            ...
            $redemptionValue = $takePackageRedemptionValue($bookingServiceId, 1);
            if ($redemptionValue > 0.0001) {
                $lineKey = $this->appointmentSettlementLineKey('service', (array) $mainLine, (int) $mainIdx);
                $persistPackageRedemptionSplits($serviceOrderItem, $bookingSplits, $lineKey, $redemptionValue, [
                    'booking_id' => (int) $booking->id,
                    'line_key' => $lineKey,
                    'line_type' => 'settlement_package_redemption',
        ...
        foreach ($addonSettlementItems as $addonIdx => $addon) {
            ...
            $addonBookingServiceId = (int) ($addon['linked_booking_service_id'] ?? 0);
            $redemptionValue = $takePackageRedemptionValue($addonBookingServiceId, $addonQty);
            if ($redemptionValue > 0.0001) {
                $lineKey = $this->appointmentSettlementLineKey('addon', (array) $addon, (int) $addonIdx);
                $persistPackageRedemptionSplits($addonOrderItem, $bookingSplits, $lineKey, $redemptionValue, [
                    'booking_id' => (int) $booking->id,
                    'line_key' => $lineKey,
                    'line_type' => 'settlement_package_redemption',
                    'service_id' => $addonBookingServiceId,
                    'redemption_value' => round($redemptionValue, 2),
                    'addon' => $addon,
                    'staff_split_source' => 'inherited',
                ]);
     ...
     */
    protected function staffSplitAmountFieldRules(string $wildcardPrefix): array
    {
        return [
            "{$wildcardPrefix}.*.share_amount" => ['nullable', 'numeric', 'min:0'],
            "{$wildcardPrefix}.*.split_mode" => ['nullable', 'string', 'in:percent,amount'],
        ];
    }
     ...
     * @return array<string, array<int, mixed>>
     */
    protected function posStaffSplitAmountValidationRules(): array
    {
        return array_merge(
            $this->staffSplitAmountFieldRules('settlement_line_staff_splits.*.staff_splits'),
            $this->staffSplitAmountFieldRules('main_service_items.*.staff_splits'),
            $this->staffSplitAmountFieldRules('main_service_items.*.addon_staff_splits.*.*'),
            $this->staffSplitAmountFieldRules('addon_staff_splits.*.*'),
            $this->staffSplitAmountFieldRules('staff_splits'),
            $this->staffSplitAmountFieldRules('items.*.staff_splits'),
            $this->staffSplitAmountFieldRules('items.*.line_staff_splits.*.staff_splits'),
            $this->staffSplitAmountFieldRules('service_items.*.line_staff_splits.*.staff_splits'),
            $this->staffSplitAmountFieldRules('package_items.*.staff_splits'),
        );
    }
}
