...
class StaffCommissionReportController extends Controller
{
    public function summary(Request $request)
    {
        ...
        $rows = $this->baseCommissionQuery($validated)
            ...
            ->selectRaw('SUM((order_items.line_total::numeric) * (order_item_staff_splits.share_percent::numeric / 100) * (CASE WHEN COALESCE(staffs.commission_rate, 0)::numeric > 1 THEN COALESCE(staffs.commission_rate, 0)::numeric / 100 ELSE COALESCE(staffs.commission_rate, 0)::numeric END)) AS total_commission')
    ...
    public function detail(Request $request)
    {
        ...
        $paginator = $this->baseCommissionQuery($validated)
            ...
            ->selectRaw('staffs.commission_rate')
            ->selectRaw('(order_items.line_total::numeric) * (order_item_staff_splits.share_percent::numeric / 100) * (CASE WHEN COALESCE(staffs.commission_rate, 0)::numeric > 1 THEN COALESCE(staffs.commission_rate, 0)::numeric / 100 ELSE COALESCE(staffs.commission_rate, 0)::numeric END) AS staff_item_commission')
