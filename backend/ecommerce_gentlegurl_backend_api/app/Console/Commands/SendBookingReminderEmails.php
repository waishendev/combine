...
class SendBookingReminderEmails extends Command
{
    ...
    protected $description = 'Send Branch-aware booking reminder emails';
    ...
    public function handle(): int
    {
        ...
        $settings = BranchNotificationSetting::query()
            ->with('storeLocation')
            ->where('booking_reminder_enabled', true)
            ->get()
            ->filter(fn (BranchNotificationSetting $setting) => $this->option('force')
                || $this->isDue($now, (string) $setting->booking_reminder_send_at));

        foreach ($settings as $setting) {
            $cacheKey = 'booking_reminder_sent_'.$setting->store_location_id.'_'.$now->toDateString();
    ...
    private function sendForBranch(BranchNotificationSetting $setting): void
    {
        ...
        $bookings = Booking::query()->with(['service', 'staff', 'customer', 'storeLocation'])
            ...
            ->whereBetween('start_at', [$start, $end])->get();
        $sent = 0;
        foreach ($bookings as $booking) {
            ...
            try {
                Mail::to($email)->queue(new BookingReminderMail(
                    customerName: $customerName, serviceName: (string) ($booking->service?->name ?? 'Service'),
                    addonItems: $addons, staffName: (string) ($booking->staff?->name ?? ''),
                    ...
                    contactPhone: (string) ($booking->storeLocation?->phone ?? ''),
                    venue: BranchEmailPresentation::from($booking->storeLocation),
                ));
                $sent++;
            } catch (\Throwable $e) {
                Log::error('Failed to queue Branch booking reminder email.', ['booking_id' => $booking->id, 'store_location_id' => $setting->store_location_id, 'error' => $e->getMessage()]);
            }
        }
        $this->info('Reminder emails queued for '.$setting->storeLocation?->name.': '.$sent);
    }
}
