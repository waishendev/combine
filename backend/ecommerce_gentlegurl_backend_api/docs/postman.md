...
- `GET /ecommerce/reports/sales/by-category` — 权限：`permission:ecommerce.reports.sales.view`。按分类汇总销售。Query：`date_from?`, `date_to?`（默认最近 30 天），`limit?`（默认 50）。
- `GET /ecommerce/reports/sales/top-products` — 权限：`permission:ecommerce.reports.sales.view`。热销产品榜。Query：`date_from?`, `date_to?`（默认最近 30 天），`limit?`（默认 20）。
- `GET /ecommerce/reports/sales/top-customers` — 权限：`permission:ecommerce.reports.sales.view`。顾客消费榜。Query：`date_from?`, `date_to?`（默认最近 30 天），`limit?`（默认 20）。
