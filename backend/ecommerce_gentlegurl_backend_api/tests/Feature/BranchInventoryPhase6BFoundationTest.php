...
class BranchInventoryPhase6BFoundationTest extends TestCase
{
    ...
    private function qty(StoreLocation $branch, Product $product, ?ProductVariant $variant = null): int { return (int) StoreLocationProductInventory::query()->where('store_location_id', $branch->id)->where('product_id', $product->id)->where('variant_identity', $variant?->id ?? 0)->value('quantity'); }
}
