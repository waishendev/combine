...
class PosPaymentMethodConfigurationTest extends TestCase {
 ...
 public function test_split_setting_is_enforced_without_changing_math(): void { $b=$this->branch('SPLIT'); foreach(PosPaymentMethod::all() as $m) DB::table('store_location_pos_payment_methods')->insert(['store_location_id'=>$b->id,'pos_payment_method_id'=>$m->id,'is_enabled'=>true,'sort_order'=>$m->default_sort_order,'created_at'=>now(),'updated_at'=>now()]); DB::table('store_location_pos_payment_settings')->insert(['store_location_id'=>$b->id,'allow_split_payment'=>false,'auto_calculate_split'=>false,'created_at'=>now(),'updated_at'=>now()]); $this->expectException(ValidationException::class); app(PosPaymentMethodService::class)->assertAllowed($b->id,[['method'=>'cash'],['method'=>'qrpay']]); }
