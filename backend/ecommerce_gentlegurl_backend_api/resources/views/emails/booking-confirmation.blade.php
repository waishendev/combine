    ...
    <div style="background-color: #f9f9f9; padding: 30px; border-radius: 8px;">
        ...
        <div style="padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px; margin: 20px 0; background-color: #ffffff;">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                @endif
                <tr>
                    <td style="padding: 8px 4px; color: #666; vertical-align:top;">Venue:</td>
                    <td style="padding: 8px 4px;">
                        @if (!empty($venue))
                            <strong>{{ $venue['name'] }}</strong><br>
                            @if (!empty($venue['address'])) {!! nl2br(e($venue['address'])) !!}<br> @endif
                            @if (!empty($venue['phone'])) {{ $venue['phone'] }} @endif
                        @else
                            <span style="color:#999;">Venue unavailable (legacy booking)</span>
                        @endif
                    </td>
                </tr>
                @if ($depositAmount > 0)
